...
class LyteDropdownComponent extends Component {
    constructor() {
        super();
        this._lyteUtilFunctions = ['toggle', 'resetPosition', 'open', 'close'];
    }
    ...
    init() {
		...
		if (!window._lyteUiUtils.dropdown) {
			window._lyteUiUtils.dropdown = { 'ind': 0 };
		}
		else {
			window._lyteUiUtils.dropdown.ind += 1;
		}
    ...
    getFreezeLayer() {
		return window._lyteUiUtils.dropdownFreezeLayer;
	}
    ...
    createFreezeLayer() {
		...
		window._lyteUiUtils.dropdownFreezeLayer = freezeLayer;

		window._lyteUiUtils.appendChild( document.body, freezeLayer );
	}
    ...
    highlightDropItemsOnMouseOver(event) {
		...
		cur = window._lyteDropdown.traverse( target, [ 'HTML', 'LYTE-DROP-ITEM' ] );
    ...
    highlightDropItemsOnHover(event) {
		var cur = event.target,
			curTar = window._lyteUiUtils.getCurrentTarget( event ),
    ...
    closeDrop1(link, event, freeze) {
		...
		window._lyteUiUtils.dispatchEvent('onhide', this.$node);
    ...
    disableItems() {
		...
		for (var i = 0; i < elements.length; i++) {
			isItemDisabled = this.$registry.registeredHelpers.lyteUiCheckDisabled(this.getData('ltPropDisabledList') || [], elements[i].dataset.value);
	...
	*/
    ...
    alignToIcon(width, offsets, doesBoxAndButtonLeftAlign) {
		var iconCls = this.getData( 'ltPropIconClass' ),
    ...
    alignToCenter(width, offsets, doesBoxAndButtonLeftAlign) {
		var link = this.childComp,
		arrow = link.querySelector( '.lyteArrow' ),
    ...
    getPositionWithHigherSpace(offsets, drop, scopeBoundary) {
    ...
    reduceHeightOfBody(preferredPosition, offsets, drop, scopeBoundary, footer_bcr, header_bcr) {
		...
		if( reduceHeight ) {
			if( this.exceedsBottomBoundary( preferredPosition, offsets, drop, scopeBoundary ) && this.exceedsTopBoundary( offsets, drop, scopeBoundary ) ) {
				...
				if( positionWithHigherSpace === 'down' ) {
					...
					body.style.height = offsets.top - scopeBoundary.top - combined_extra_height + 'px';
				}
			}
    ...
    getFocusableElementsOfBox() {
		var box = this.getDropBox();

		return box.querySelectorAll( window._lyteDropdown.focusableElementsSelector );
	}

    getFocusableElementsOfButton() {
		var node = this.$node;

		return node.querySelectorAll( window._lyteDropdown.focusableElementsSelector );
	}
    ...
    isFirstFocusableElement() {
		var box = this.getDropBox(),
		elements = box.querySelectorAll( window._lyteDropdown.focusableElementsSelector ),
		activeElement = document.activeElement;

		return elements[ 0 ] === activeElement;
	}

    focusFirstFocusableBoxElement() {
		var focusableElementsOfBox = this.getFocusableElementsOfBox();

		focusableElementsOfBox[ 0 ].focus();
	}

    focusProperElement(event) {
		var focusableElements = this.getFocusableElements(),
    ...
    setLast(node) {
		var prev = window._lyteUiUtils.querySelector('[lyte-last="true"]');
    ...
    data(arg1) {
		 ...
		 * @default { "position" : "bottom", "appearance" : "box","margin" : 5, "keeptooltip" : true}
			...
			'ltPropIncludeButtonInFocusLoop': prop( 'boolean', { 'default': false } ),

			'ltPropForceCalculateWidth': prop( 'boolean', { 'default': false } ),

			'ltPropSetAriaOnActiveElement': prop( 'boolean', { 'default': false } ),

			'ltPropCaseSensitive': prop( 'boolean', { 'default': false } ),

			'ltPropSearchMethod': prop('string', { 'default': 'contains' }),
			'ltPropLabel': prop('string', { 'default': '' }),
			'ltPropLabelClass': prop('string', { 'default': '' }),
			'ltPropDirection': prop('string', { default: 'vertical' }),

			'ltPropFocusName': prop( 'string', { 'default': '' } ),

			'ltPropBfocusName': prop( 'string', { 'default': '' } ),

			'ltPropNfocusName': prop( 'string', { 'default': '' } ),

			'ltPropUseItemOffsetForScroll': prop( 'boolean', { 'default': false } ),

			'ltPropReduceHeightOfBody': prop( 'boolean', { 'default': false } ),

			'ltPropAlignCalloutToIcon': prop( 'boolean', { 'default': false } ),
...
LyteDropdownComponent._template = "<template tag-name=\"lyte-dropdown\"> <template is=\"switch\" l-c=\"true\" _jsp=\"true\"><template is=\"case\" case=\"{{expHandlers(ltPropHover,'==',true)}}\" lc-id=\"lc_id_0\"> <template is=\"switch\" l-c=\"true\" _jsp=\"true\"><template is=\"case\" case=\"{{expHandlers(multiple,'==',true)}}\" lc-id=\"lc_id_0\"> <template is=\"switch\" l-c=\"true\" _jsp=\"true\"><template is=\"case\" case=\"{{expHandlers(search,'==',true)}}\" lc-id=\"lc_id_0\"> <template is=\"switch\" l-c=\"true\" _jsp=\"true\"><template is=\"case\" case=\"{{expHandlers(drophead,'==','noyield')}}\" lc-id=\"lc_id_0\"> <lyte-drop-button id=\"{{ltPropId}}\" role=\"combobox\" aria-expanded=\"{{if(ltPropIsOpen,'true','false')}}\" aria-haspopup=\"listbox\"> <div class=\"lyteMultiselect\"> <ul class=\"lyteMultipleSelect\"> <template is=\"for\" _jsp=\"true\" items=\"{{ltPropSelectedList}}\" item=\"item\" index=\"indexVal\"> <li data-value=\"{{lyteUiReturnValueBy(item,sysValue)}}\"> <span class=\"lyteDropdownVisible\" onmouseenter=\"{{action('toolTipConfig',this,'selToolTip')}}\" lt-prop-title=\"{{if(selToolTip,lyteUiReturnValueBy(item,userValue),'')}}\" lt-prop-tooltip-config=\"{{ltPropTooltip}}\" lt-prop-tooltip-class=\"{{ltPropTooltipClass}}\">{{lyteUiReturnValueBy(item,userValue)}}</span> <lyte-drop-remove class=\"lyteCloseIcon\"></lyte-drop-remove> </li> </template> <li class=\"lyteMultiselectInput\"> <input disabled=\"{{ltPropDisabled}}\" readonly=\"{{ltPropReadOnly}}\" onkeydown=\"{{action('preventDefault',event)}}\" type=\"text\" placeholder=\"{{ltPropPlaceholder}}\" class=\"lyteDropdownTextField\" autocomplete=\"off\" tabindex=\"{{ltPropTabindex}}\" data-tabindex=\"{{ltPropDataTabindex}}\" onclick=\"{{action('showHide',event)}}\" onfocus=\"{{action('addFocusClass',event)}}\" onblur=\"{{action('removeFocusClass',event)}}\"> </li> </ul> </div> <template is=\"switch\" l-c=\"true\" _jsp=\"true\"><template is=\"case\" case=\"{{shouldDisplayIcon}}\" lc-id=\"lc_id_0\"> <lyte-icon class=\"{{ltPropIconClass}}\"></lyte-icon> </template></template> </lyte-drop-button> </template><template default=\"\"> </template></template> </template><template default=\"\"> <template is=\"switch\" l-c=\"true\" _jsp=\"true\"><template is=\"case\" case=\"{{expHandlers(drophead,'==',&quot;noyield&quot;)}}\" lc-id=\"lc_id_0\"> <div id=\"{{ltPropId}}\" class=\"lyteDummyEventContainer\" role=\"combobox\" aria-haspopup=\"listbox\" aria-expanded=\"{{if(ltPropIsOpen,'true','false')}}\" onclick=\"{{action('showHide',event)}}\" onkeyup=\"{{action('checkKey',event)}}\" tabindex=\"{{ltPropTabindex}}\" data-tabindex=\"{{ltPropDataTabindex}}\"> <lyte-drop-button> <template is=\"switch\" l-c=\"true\" _jsp=\"true\"><template is=\"case\" case=\"{{expHandlers(expHandlers(ltPropSelectedList.length,'==',0),'&amp;&amp;',expHandlers(ltPropForcePlaceholder,'!'))}}\" lc-id=\"lc_id_0\"> <span class=\"lyteDropPlaceholderMultiple\">{{ltPropPlaceholder}}</span> </template></template> <ul class=\"lyteMultipleSelect\"> <template is=\"for\" _jsp=\"true\" items=\"{{ltPropSelectedList}}\" item=\"item\" index=\"indexVal\"> <li data-value=\"{{lyteUiReturnValueBy(item,sysValue)}}\"> <span class=\"lyteDropdownVisible\" onmouseenter=\"{{action('toolTipConfig',this,'selToolTip')}}\" lt-prop-title=\"{{if(selToolTip,lyteUiReturnValueBy(item,userValue),'')}}\" lt-prop-tooltip-config=\"{{ltPropTooltip}}\" lt-prop-tooltip-class=\"{{ltPropTooltipClass}}\">{{lyteUiReturnValueBy(item,userValue)}}</span> <lyte-drop-remove class=\"lyteCloseIcon\"></lyte-drop-remove> </li> </template> </ul> <template is=\"switch\" l-c=\"true\" _jsp=\"true\"><template is=\"case\" case=\"{{ltPropForcePlaceholder}}\" lc-id=\"lc_id_0\"> <span class=\"lyteDropPlaceholderMultiple\">{{ltPropPlaceholder}}</span> </template></template> <template is=\"switch\" l-c=\"true\" _jsp=\"true\"><template is=\"case\" case=\"{{shouldDisplayIcon}}\" lc-id=\"lc_id_0\"> <lyte-icon class=\"{{ltPropIconClass}}\"></lyte-icon> </template></template> </lyte-drop-button> </div> </template><template default=\"\"> </template></template> </template></template> </template><template default=\"\"> <div id=\"{{ltPropId}}\" onmouseenter=\"{{action('showHide',event,'enter')}}\" role=\"combobox\" aria-haspopup=\"listbox\" aria-expanded=\"{{if(ltPropIsOpen,'true','false')}}\" onmouseleave=\"{{action('closeIt',event)}}\" class=\"lyteDummyEventContainer\" tabindex=\"{{ltPropTabindex}}\" data-tabindex=\"{{ltPropDataTabindex}}\" style=\"\"> <template is=\"switch\" l-c=\"true\" _jsp=\"true\"><template is=\"case\" case=\"{{expHandlers(drophead,'==',&quot;noyield&quot;)}}\" lc-id=\"lc_id_0\"> <lyte-drop-button> <template is=\"switch\" l-c=\"true\" _jsp=\"true\"><template is=\"case\" case=\"{{ltPropDisplayValue}}\" lc-id=\"lc_id_0\"> <span class=\"lyteMarginRight lyteDropdownLabel\" onmouseenter=\"{{action('toolTipConfig',this,'showToolTip')}}\" lt-prop-title=\"{{if(showToolTip,ltPropDisplayValue,'')}}\" lt-prop-tooltip-config=\"{{ltPropTooltip}}\" lt-prop-tooltip-class=\"{{ltPropTooltipClass}}\">{{ltPropDisplayValue}}</span> </template><template is=\"case\" case=\"{{expHandlers(expHandlers(ltPropSelected,'!'),'&amp;&amp;',expHandlers(ltPropDisplayValue,'!'))}}\" lc-id=\"lc_id_1\"> <span class=\"lyteDropPlaceholderNormal\">{{ltPropPlaceholder}}</span> </template></template> <template is=\"switch\" l-c=\"true\" _jsp=\"true\"><template is=\"case\" case=\"{{expHandlers(ltPropSelected,'&amp;&amp;',ltPropShowRemoveIcon)}}\" lc-id=\"lc_id_0\"> <lyte-drop-remove class=\"lyteDropdownDeselectIcon\"></lyte-drop-remove> </template></template> <lyte-icon class=\"{{ltPropIconClass}}\"></lyte-icon> </lyte-drop-button> </template><template is=\"case\" case=\"{{expHandlers(drophead,'==',&quot;yield&quot;)}}\" lc-id=\"lc_id_1\"> </template></template> </div> </template></template> </template><template default=\"\"> <template is=\"switch\" l-c=\"true\" _jsp=\"true\"><template is=\"case\" case=\"{{expHandlers(multiple,'==',true)}}\" lc-id=\"lc_id_0\"> <template is=\"switch\" l-c=\"true\" _jsp=\"true\"><template is=\"case\" case=\"{{expHandlers(search,'==',true)}}\" lc-id=\"lc_id_0\"> <template is=\"switch\" l-c=\"true\" _jsp=\"true\"><template is=\"case\" case=\"{{expHandlers(drophead,'==','noyield')}}\" lc-id=\"lc_id_0\"> <lyte-drop-button id=\"{{ltPropId}}\" onclick=\"{{action('showHide',event)}}\" role=\"combobox\" aria-haspopup=\"listbox\" aria-expanded=\"{{if(ltPropIsOpen,'true','false')}}\"> <div class=\"lyteMultiselect\"> <ul class=\"lyteMultipleSelect\"> <template is=\"for\" _jsp=\"true\" items=\"{{ltPropSelectedList}}\" item=\"item\" index=\"indexVal\"> <li data-value=\"{{lyteUiReturnValueBy(item,sysValue)}}\"> <span class=\"lyteDropdownVisible\" onmouseenter=\"{{action('toolTipConfig',this,'selToolTip')}}\" lt-prop-title=\"{{if(selToolTip,lyteUiReturnValueBy(item,userValue),'')}}\" lt-prop-tooltip-config=\"{{ltPropTooltip}}\" lt-prop-tooltip-class=\"{{ltPropTooltipClass}}\">{{lyteUiReturnValueBy(item,userValue)}}</span> <lyte-drop-remove class=\"lyteCloseIcon\"></lyte-drop-remove> </li> </template> <li class=\"lyteMultiselectInput\"> <input disabled=\"{{ltPropDisabled}}\" readonly=\"{{ltPropReadOnly}}\" onkeydown=\"{{action('preventDefault',event)}}\" type=\"text\" placeholder=\"{{ltPropPlaceholder}}\" class=\"lyteDropdownTextField\" autocomplete=\"off\" tabindex=\"{{ltPropTabindex}}\" data-tabindex=\"{{ltPropDataTabindex}}\" onfocus=\"{{action('addFocusClass',event)}}\" onblur=\"{{action('removeFocusClass',event)}}\"> </li> </ul> </div> <template is=\"switch\" l-c=\"true\" _jsp=\"true\"><template is=\"case\" case=\"{{shouldDisplayIcon}}\" lc-id=\"lc_id_0\"> <lyte-icon class=\"{{ltPropIconClass}}\"></lyte-icon> </template></template> </lyte-drop-button> </template><template default=\"\"> <div id=\"{{ltPropId}}\" class=\"lyteDummyEventContainer\" style=\"\" onclick=\"{{action('showHide',event)}}\" role=\"combobox\" aria-haspopup=\"listbox\" aria-expanded=\"{{if(ltPropIsOpen,'true','false')}}\"> </div> </template></template> </template><template default=\"\"> <div id=\"{{ltPropId}}\" class=\"lyteDummyEventContainer\" onclick=\"{{action('showHide',event)}}\" tabindex=\"{{ltPropTabindex}}\" data-tabindex=\"{{ltPropDataTabindex}}\" role=\"combobox\" aria-haspopup=\"listbox\" aria-expanded=\"{{if(ltPropIsOpen,'true','false')}}\"> <template is=\"switch\" l-c=\"true\" _jsp=\"true\"><template is=\"case\" case=\"{{expHandlers(drophead,'==',&quot;noyield&quot;)}}\" lc-id=\"lc_id_0\"> <lyte-drop-button> <template is=\"switch\" l-c=\"true\" _jsp=\"true\"><template is=\"case\" case=\"{{expHandlers(expHandlers(ltPropSelectedList.length,'==',0),'&amp;&amp;',expHandlers(ltPropForcePlaceholder,'!'))}}\" lc-id=\"lc_id_0\"> <span class=\"lyteDropPlaceholderMultiple\">{{ltPropPlaceholder}}</span> </template></template> <ul class=\"lyteMultipleSelect\"> <template is=\"for\" _jsp=\"true\" items=\"{{ltPropSelectedList}}\" item=\"item\" index=\"indexVal\"> <li data-value=\"{{lyteUiReturnValueBy(item,sysValue)}}\"> <span class=\"lyteDropdownVisible\" onmouseenter=\"{{action('toolTipConfig',this,'selToolTip')}}\" lt-prop-title=\"{{if(selToolTip,lyteUiReturnValueBy(item,userValue),'')}}\" lt-prop-tooltip-config=\"{{ltPropTooltip}}\" lt-prop-tooltip-class=\"{{ltPropTooltipClass}}\">{{lyteUiReturnValueBy(item,userValue)}}</span> <lyte-drop-remove class=\"lyteCloseIcon\"></lyte-drop-remove> </li> </template> </ul> <template is=\"switch\" l-c=\"true\" _jsp=\"true\"><template is=\"case\" case=\"{{ltPropForcePlaceholder}}\" lc-id=\"lc_id_0\"> <span class=\"lyteDropPlaceholderMultiple\">{{ltPropPlaceholder}}</span> </template></template> <template is=\"switch\" l-c=\"true\" _jsp=\"true\"><template is=\"case\" case=\"{{shouldDisplayIcon}}\" lc-id=\"lc_id_0\"> <lyte-icon class=\"{{ltPropIconClass}}\"></lyte-icon> </template></template> </lyte-drop-button> </template><template default=\"\"> </template></template> </div> </template></template> </template><template default=\"\"> <div id=\"{{ltPropId}}\" onclick=\"{{action('showHide',event)}}\" class=\"lyteDummyEventContainer\" tabindex=\"{{ltPropTabindex}}\" data-tabindex=\"{{ltPropDataTabindex}}\" style=\"\" role=\"combobox\" aria-haspopup=\"listbox\" aria-expanded=\"{{if(ltPropIsOpen,'true','false')}}\"> <template is=\"switch\" l-c=\"true\" _jsp=\"true\"><template is=\"case\" case=\"{{expHandlers(drophead,'==',&quot;noyield&quot;)}}\" lc-id=\"lc_id_0\"> <lyte-drop-button> <template is=\"switch\" l-c=\"true\" _jsp=\"true\"><template is=\"case\" case=\"{{ltPropDisplayValue}}\" lc-id=\"lc_id_0\"> <span onmouseenter=\"{{action('toolTipConfig',this,'showToolTip')}}\" lt-prop-tooltip-config=\"{{ltPropTooltip}}\" lt-prop-tooltip-class=\"{{ltPropTooltipClass}}\" lt-prop-title=\"{{if(showToolTip,ltPropDisplayValue,'')}}\" class=\"lyteMarginRight lyteDropdownLabel\">{{ltPropDisplayValue}}</span> </template><template is=\"case\" case=\"{{expHandlers(expHandlers(ltPropSelected,'!'),'&amp;&amp;',expHandlers(ltPropDisplayValue,'!'))}}\" lc-id=\"lc_id_1\"> <span class=\"lyteDropPlaceholderNormal\">{{ltPropPlaceholder}}</span> </template></template> <template is=\"switch\" l-c=\"true\" _jsp=\"true\"><template is=\"case\" case=\"{{expHandlers(ltPropSelected,'&amp;&amp;',ltPropShowRemoveIcon)}}\" lc-id=\"lc_id_0\"> <lyte-drop-remove class=\"lyteDropdownDeselectIcon\"></lyte-drop-remove> </template></template> <lyte-icon class=\"{{ltPropIconClass}}\"></lyte-icon> </lyte-drop-button> </template><template is=\"case\" case=\"{{expHandlers(drophead,'==',&quot;yield&quot;)}}\" lc-id=\"lc_id_1\"> </template></template> </div> </template></template> </template></template> <template is=\"switch\" l-c=\"true\" _jsp=\"true\"><template is=\"case\" case=\"{{expHandlers(ltPropHover,'==',false)}}\" lc-id=\"lc_id_0\"> <template is=\"switch\" l-c=\"true\" _jsp=\"true\"><template is=\"case\" case=\"{{expHandlers(dropbody,'==',&quot;noyield&quot;)}}\" lc-id=\"lc_id_0\"> <lyte-drop-box class=\"lyteDropdownHidden\"> <lyte-drop-body> <template is=\"for\" _jsp=\"true\" items=\"{{ltPropOptions}}\" item=\"item\" index=\"indexVal\"><template is=\"switch\" l-c=\"true\" _jsp=\"true\"><template is=\"case\" case=\"{{lyteUiOptGroupCheck(item)}}\" lc-id=\"lc_id_0\"> <lyte-drop-group> <lyte-drop-label>{{lyteUiReturnOnlyKey(item)}}</lyte-drop-label> <template is=\"for\" _jsp=\"true\" items=\"{{lyteUiReturnValueBy(item,lyteUiReturnOnlyKey(item))}}\" item=\"subitem\" index=\"indexval\"> <template is=\"switch\" l-c=\"true\" _jsp=\"true\"><template is=\"case\" case=\"{{lyteUiIsObject(subitem)}}\" lc-id=\"lc_id_0\"> <lyte-drop-item data-value=\"{{subitem[ltPropSystemValue]}}\" disabled=\"{{lyteUiCheckDisabled(ltPropDisabledList,subitem[ltPropSystemValue])}}\">{{subitem[ltPropUserValue]}}</lyte-drop-item> </template><template default=\"\"> <lyte-drop-item data-value=\"{{subitem}}\" disabled=\"{{lyteUiCheckDisabled(ltPropDisabledList,subitem)}}\">{{subitem}}</lyte-drop-item> </template></template> </template> </lyte-drop-group> </template><template default=\"\"> <template is=\"switch\" l-c=\"true\" _jsp=\"true\"><template is=\"case\" case=\"{{lyteUiIsObject(item)}}\" lc-id=\"lc_id_0\"> <lyte-drop-item data-value=\"{{item[ltPropSystemValue]}}\" disabled=\"{{lyteUiCheckDisabled(ltPropDisabledList,item[ltPropSystemValue])}}\">{{item[ltPropUserValue]}}</lyte-drop-item> </template><template default=\"\"> <lyte-drop-item data-value=\"{{item}}\" disabled=\"{{lyteUiCheckDisabled(ltPropDisabledList,item)}}\">{{item}}</lyte-drop-item> </template></template> </template></template> </template> </lyte-drop-body> </lyte-drop-box> </template><template default=\"\"> </template></template> <div class=\"lyteLoadMsg\" style=\"display:none;\">Loading</div> </template><template default=\"\"> <template is=\"switch\" l-c=\"true\" _jsp=\"true\"><template is=\"case\" case=\"{{expHandlers(dropbody,'==',&quot;noyield&quot;)}}\" lc-id=\"lc_id_0\"> <lyte-drop-box class=\"lyteDropdownHidden\"> <lyte-drop-body> <template is=\"for\" _jsp=\"true\" items=\"{{ltPropOptions}}\" item=\"item\" index=\"indexVal\"><template is=\"switch\" l-c=\"true\" _jsp=\"true\"><template is=\"case\" case=\"{{lyteUiOptGroupCheck(item)}}\" lc-id=\"lc_id_0\"> <lyte-drop-group> <lyte-drop-label>{{lyteUiReturnOnlyKey(item)}}</lyte-drop-label> <template is=\"for\" _jsp=\"true\" items=\"{{lyteUiReturnValueBy(item,lyteUiReturnOnlyKey(item))}}\" item=\"subitem\" index=\"indexval\"> <template is=\"switch\" l-c=\"true\" _jsp=\"true\"><template is=\"case\" case=\"{{lyteUiIsObject(subitem)}}\" lc-id=\"lc_id_0\"> <lyte-drop-item data-value=\"{{subitem[ltPropSystemValue]}}\" disabled=\"{{lyteUiCheckDisabled(ltPropDisabledList,subitem[ltPropSystemValue])}}\">{{subitem[ltPropUserValue]}}</lyte-drop-item> </template><template default=\"\"> <lyte-drop-item data-value=\"{{subitem}}\" disabled=\"{{lyteUiCheckDisabled(ltPropDisabledList,subitem)}}\">{{subitem}}</lyte-drop-item> </template></template> </template> </lyte-drop-group> </template><template default=\"\"> <template is=\"switch\" l-c=\"true\" _jsp=\"true\"><template is=\"case\" case=\"{{lyteUiIsObject(item)}}\" lc-id=\"lc_id_0\"> <lyte-drop-item data-value=\"{{item[ltPropSystemValue]}}\" disabled=\"{{lyteUiCheckDisabled(ltPropDisabledList,item[ltPropSystemValue])}}\">{{item[ltPropUserValue]}}</lyte-drop-item> </template><template default=\"\"> <lyte-drop-item data-value=\"{{item}}\" disabled=\"{{lyteUiCheckDisabled(ltPropDisabledList,item)}}\">{{item}}</lyte-drop-item> </template></template> </template></template> </template> </lyte-drop-body> </lyte-drop-box> </template><template default=\"\"> </template></template> <div class=\"lyteLoadMsg\" style=\"display:none;\">Loading</div> </template></template> <lyte-yield yield-name=\"yield\" style=\"display: none;\" lyte-options=\"{{ltPropOptions}}\"></lyte-yield> </template>";;
...
LyteDropdownComponent._observedAttributes = [
    ...
    "ltPropNoResultYield",
    "ltPropIncludeButtonInFocusLoop",
    "ltPropForceCalculateWidth",
    "ltPropSetAriaOnActiveElement",
    "ltPropCaseSensitive",
    "ltPropSearchMethod",
    "ltPropLabel",
    "ltPropLabelClass",
    ...
    "ltPropBfocusName",
    "ltPropNfocusName",
    "ltPropUseItemOffsetForScroll",
    "ltPropReduceHeightOfBody",
    "ltPropAlignCalloutToIcon",
    "ltPropId"
];

window._lyteUiUtils.addGlobalEventListener('scroll', function (event) {
	var cur = event.target;
...
window._lyteUiUtils.addGlobalEventListener('click', function (event) {
...
if (window._lyteUiUtils.isIos) {
	window._lyteUiUtils.addGlobalEventListener('focusout', function (event) {
		...
		if ((drop = window._lyteDropdown.traverse(target, 'LYTE-DROPDOWN'))
			...
			setTimeout(function () {
				...
				if (window._lyteDropdown.preventClose) {
					window._lyteDropdown.preventClose = window._lyteDropdown.focusOut = false;
					return;
				}
				...
				window._lyteDropdown.focusOut = false;
			}, 0);
		}
	}, true);
}
...
window._lyteUiUtils.addGlobalEventListener('keydown', function (event) {  //This is to take care of the tabbing problems in a dropdown and traversing(CODE HELP)
...
window._lyteUiUtils.addGlobalEventListener('keypress', function (event) { //It searches the dropdown items matching the pressed charactes when the dropdown is open
	...
	if (
		...
		window._lyteDropdown.pressedCharacter = window._lyteDropdown.pressedCharacter ? window._lyteDropdown.pressedCharacter += String.fromCharCode(kc) : String.fromCharCode(kc);
		if (window._lyteDropdown.checkDDtimeoutId) {
			clearTimeout(window._lyteDropdown.checkDDtimeoutId);
		}

		window._lyteDropdown.checkDDtimeoutId = setTimeout(function () {
			window._lyteDropdown.pressedCharacter = null;
		}, 500);
		...
		if (open.length > 0 && drp) {
			...
			if (
				window._lyteDropdown.cachePreviousVal.char
				&& window._lyteDropdown.cachePreviousVal.char == window._lyteDropdown.pressedCharacter
				&& (window._lyteDropdown.cachePreviousVal.pos + 1) < children.length
				&& isMatchFound(children[window._lyteDropdown.cachePreviousVal.pos + 1].textContent, window._lyteDropdown.pressedCharacter, searchMethod)/* children[ _lyteDropdown.cachePreviousVal.pos + 1 ].textContent.trim().substring( 0, _lyteDropdown.pressedCharacter.length ).toLowerCase() === _lyteDropdown.pressedCharacter.toLowerCase()*/
			) {
				pos = window._lyteDropdown.cachePreviousVal.pos + 1;
			}
			else {
				for (i = 0; i < children.length; i++) {
					if (isMatchFound(children[i].textContent, window._lyteDropdown.pressedCharacter, searchMethod) /*children[ i ].textContent.trim().substring( 0, _lyteDropdown.pressedCharacter.length ).toLowerCase() === _lyteDropdown.pressedCharacter.toLowerCase()*/) {
						pos = i;
						break;
					}
				}
			...
			window._lyteDropdown.cachePreviousVal.char = window._lyteDropdown.pressedCharacter;
			window._lyteDropdown.cachePreviousVal.pos = pos;
		}
        ...
        connectedCallback() {
            ...
            if (!lyteNode._eventRegistered) {
				...
				lyteNode.addEventListener('mouseenter', function () {
					...
					var isOverflowVisible = window.getComputedStyle( lyteNode ).overflow === 'visible';

					showToolTip = (!isOverflowVisible && width < scrollWidth && !window._lyteDropdown.isEmpty(tooltipConfig));
        ...
        attributeChangedCallback(attr, oldValue, newValue, namespace) {
            ...
            if (attr == 'selected') {
				...
				if( ( oldValue || "" ).indexOf( name ) + 1 || ( newValue || "" ).indexOf( name ) + 1 ){
					this.common_check( window._lyteDropdown.getDropdownFromItem( lyteNode ), "both" );
				}
			}
        }
...
window._lyteDropdown = {
	...
	getVisibleDropbox: function () {
		...
		for (; i < childs.length; i++) {
			if (
				...
				&& ( box = window._lyteDropdown.getInnerBox( childs[ i ] ) ) 
				&& window._lyteDropdown.isVisible( box ) 
			) {
				return box;
			}
		}
	...
	openAutoClosedDropdown: function (cur, event) {
		var isDocument = window._lyteDropdown.isDocument( cur ),
	...
	getActive: function () {
		var node = window._lyteUiUtils.querySelector( '.lyteDropMark' );
