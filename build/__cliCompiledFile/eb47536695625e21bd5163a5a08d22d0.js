...
window.addEventListener('click' , function(eve){
	...
	if(window._lyteUiUtils.popupStack){
		if(window._lyteUiUtils.popupStack.globalStack && window._lyteUiUtils.popupStack.betaPopoverStack.length > 0){
			var lastPopup = window._lyteUiUtils.popupStack.globalStack[ window._lyteUiUtils.popupStack.globalStack.length - 1 ]
			if(
				(window._lyteUiUtils.popupStack.globalStack[window._lyteUiUtils.popupStack.globalStack.length-1].parentElement === window._lyteUiUtils.popupStack.betaPopoverStack[window._lyteUiUtils.popupStack.betaPopoverStack.length-1].parentElement) ||
				(window._lyteUiUtils.popupStack.globalStack[window._lyteUiUtils.popupStack.globalStack.length-1].parentElement.tagName === 'LYTE-POPOVER')
				){
				if(lastPopup.parentElement.tagName === "LYTE-BETA-POPOVER" && $L(lastPopup.parentElement).hasClass('lytePopoverOpened') && ($L(eve.target).attr('id') !== 'lytedropdownfreezelayer') && (eve.target.tagName !== 'LYTE-DROP-ITEM') && ($L(eve.target).closest('lyte-wormhole')[0] == lastPopup.childElement)){
					...
					lastPopup = window._lyteUiUtils.popupStack.betaPopoverStack[ window._lyteUiUtils.popupStack.betaPopoverStack.length - 1 ]
					if( ( window._lyteUiUtils.popupStack.betaPopoverStack[ window._lyteUiUtils.popupStack.betaPopoverStack.length - 1 ] !== window._lyteUiUtils.popupStack.globalStack[ window._lyteUiUtils.popupStack.globalStack.length - 1 ]) || !lastPopup.parentElement.getData('ltPropCloseOnBodyClick')){
						return
					}
					lastPopup.parentElement.setData('ltPropShow' , false);
				}
...
_lyteBetaPopover.scrollHandler = function(event){
	...
	if(window._lyteUiUtils.targetAvailable){
		if(window._lyteUiUtils.targetAvailable.originElem){
			var originElem = $L(window._lyteUiUtils.targetAvailable.originElem)[0]
			if(window._lyteUiUtils.targetAvailable && window._lyteUiUtils.targetAvailable.closedOn){
				if(window._lyteUiUtils.targetAvailable && window._lyteUiUtils.targetAvailable.closedOn === 'top'){
					if(window._lyteUiUtils.targetAvailable.top && originElem.getBoundingClientRect().top >= window._lyteUiUtils.targetAvailable.top){
						window._lyteUiUtils.targetAvailable.popover.setData('ltPropShow' , true)
						delete window._lyteUiUtils.targetAvailable
					}
				} else if(window._lyteUiUtils.targetAvailable && window._lyteUiUtils.targetAvailable.closedOn === 'bottom'){
					if(window._lyteUiUtils.targetAvailable.bottom && (window.innerHeight - (originElem.getBoundingClientRect().top + originElem.getBoundingClientRect().height)) >= window._lyteUiUtils.targetAvailable.bottom){
						window._lyteUiUtils.targetAvailable.popover.setData('ltPropShow' , true)
						delete window._lyteUiUtils.targetAvailable
					}
				}
				if(window._lyteUiUtils.targetAvailable && window._lyteUiUtils.targetAvailable.closedOn === 'left'){
					if(window._lyteUiUtils.targetAvailable.left && originElem.getBoundingClientRect().left >= window._lyteUiUtils.targetAvailable.left){
						window._lyteUiUtils.targetAvailable.popover.setData('ltPropShow' , true)
						delete window._lyteUiUtils.targetAvailable
					}
				} else if(window._lyteUiUtils.targetAvailable &&window._lyteUiUtils.targetAvailable.closedOn === 'right'){
					if(window._lyteUiUtils.targetAvailable.right && (window.innerWidth - (originElem.getBoundingClientRect().left + originElem.getBoundingClientRect().width)) >= window._lyteUiUtils.targetAvailable.right){
						window._lyteUiUtils.targetAvailable.popover.setData('ltPropShow' , true)
						delete window._lyteUiUtils.targetAvailable
					}
				}
			...
			for(var i=0;i<window._lyteUiUtils.popupStack.betaPopoverStack.length;i++){
				...
				if(currentPopup){
					...
					if(!currentPopup.component.isPopoverInBoundary()){
						if(!window._lyteUiUtils.targetAvailable){
							window._lyteUiUtils.targetAvailable = {}
						}
						window._lyteUiUtils.targetAvailable.popover = currentPopup
						if(currentPopup.getData('ltPropBoundary').top){
							window._lyteUiUtils.targetAvailable.top = parseFloat(currentPopup.getData('ltPropBoundary').top)
						} else {
							window._lyteUiUtils.targetAvailable.top = 0
						}
						if(currentPopup.getData('ltPropBoundary').left){
							window._lyteUiUtils.targetAvailable.left = parseFloat(currentPopup.getData('ltPropBoundary').left)
						} else {
							window._lyteUiUtils.targetAvailable.left = 0
						}
						if(currentPopup.getData('ltPropBoundary').bottom){
							window._lyteUiUtils.targetAvailable.bottom = parseFloat(currentPopup.getData('ltPropBoundary').bottom)
						} else {
							window._lyteUiUtils.targetAvailable.bottom = window.innerHeight
						}
						if(currentPopup.getData('ltPropBoundary').right){
							window._lyteUiUtils.targetAvailable.right = parseFloat(currentPopup.getData('ltPropBoundary').right)
						} else {
							window._lyteUiUtils.targetAvailable.right = window.innerWidth
						}
						window._lyteUiUtils.targetAvailable.originElem = currentPopup.getData('ltPropOriginElem')
						currentPopup.setData('ltPropShow' , false);
						return;
					}
...
class LyteBetaPopoverComponent extends Component {
    ...
    data(arg1) {
		return Object.assign(super.data({
			...
			ltPropIgnoreBoundary : prop( 'boolean' ,{	
				default : false
			}),
		...
		if(!window._lyteUiUtils.lytePopoverKeyDown){
			...
			document.addEventListener('keydown' , function(evt){
				...
				if(window._lyteUiUtils.popupStack && window._lyteUiUtils.popupStack.betaPopoverStack){
					popoverArr = window._lyteUiUtils.popupStack.betaPopoverStack
				}
				if(window._lyteUiUtils.popupStack && window._lyteUiUtils.popupStack.globalStack){
					popStack = window._lyteUiUtils.popupStack.globalStack
				}
				var isEscape = false;
				if ("key" in evt) {
    ...
    mousedownFun(eve) {
		...
		objectUtils(_this.getData('initialComputedPopoverDim') , 'add' , 'left' , parseFloat(getComputedStyle($L(_this.actualModalDiv)[0]).left))
		objectUtils(_this.getData('initialComputedPopoverDim') , 'add' , 'top' , parseFloat(getComputedStyle($L(_this.actualModalDiv)[0]).top))
    ...
    closePrevPopup() {
		if(window._lyteUiUtils.popupStack && window._lyteUiUtils.popupStack.globalStack && window._lyteUiUtils.popupStack.globalStack.length > 0){
			var tagname = window._lyteUiUtils.popupStack.globalStack[window._lyteUiUtils.popupStack.globalStack.length-1].parentElement.tagName
				if(
					tagname === 'LYTE-BETA-POPOVER' || 
					tagname === 'LYTE-BETA-MODAL' ||
					tagname === 'LYTE-POPOVER' ||
					tagname === 'LYTE-MODAL'){
						var prevIndex = window._lyteUiUtils.popupStack.globalStack.length-1
						if(window._lyteUiUtils.popupStack.globalStack[prevIndex].parentElement.getData('ltPropAllowMultiple') === false){
							window._lyteUiUtils.popupStack.globalStack[prevIndex].parentElement.setData('ltPropShow' , false);
						}
					}
		}
	}
			...
			if((originDim.top < 0) || (originDim.top + originDim.height > window.innerHeight)){
				if(this.getData('ltPropReOpen')){
					if(!window._lyteUiUtils.targetAvailable){
						window._lyteUiUtils.targetAvailable = {}
						if((originDim.top < 0)){
							window._lyteUiUtils.targetAvailable.closedOn = 'top'
						} else if((originDim.top + originDim.height > window.innerHeight)){
							window._lyteUiUtils.targetAvailable.closedOn = 'bottom'
						}
						window._lyteUiUtils.targetAvailable.popover = this
						if(this.getData('ltPropBoundary').top){
							window._lyteUiUtils.targetAvailable.top = parseFloat(this.getData('ltPropBoundary').top)
						} else {
							window._lyteUiUtils.targetAvailable.top = 0
						}
						if(this.getData('ltPropBoundary').bottom){
							window._lyteUiUtils.targetAvailable.bottom = parseFloat(this.getData('ltPropBoundary').bottom)
						} else {
							window._lyteUiUtils.targetAvailable.bottom = window.innerHeight
						}

						window._lyteUiUtils.targetAvailable.originElem = this.getData('ltPropOriginElem')
			...
			if((originDim.left < 0) || (originDim.left + originDim.width > window.innerWidth)){
				if(this.getData('ltPropReOpen')){
					if(!window._lyteUiUtils.targetAvailable){
						window._lyteUiUtils.targetAvailable = {}
						if((originDim.left < 0)){
							window._lyteUiUtils.targetAvailable.closedOn = 'left'
						} else if((originDim.left + originDim.width > window.innerWidth)){
							window._lyteUiUtils.targetAvailable.closedOn = 'right'
						}
		
						if(this.getData('ltPropBoundary').right){
							window._lyteUiUtils.targetAvailable.right = parseFloat(this.getData('ltPropBoundary').right)
						} else {
							window._lyteUiUtils.targetAvailable.right = window.innerWidth
						}
		
						if(this.getData('ltPropBoundary').left){
							window._lyteUiUtils.targetAvailable.left = parseFloat(this.getData('ltPropBoundary').left)
						} else {
							window._lyteUiUtils.targetAvailable.left = 0
						}
						window._lyteUiUtils.targetAvailable.originElem = this.getData('ltPropOriginElem')
    ...
    _closePopover(event) {
		var popStack = window._lyteUiUtils.popupStack.globalStack
		var popoverElem = $L(this.childComp).find('.lytePopoverWrapper')
		var popoverElemInnerElem = popoverElem.find('.lytePopoverElement')[0];
		var animationType = this.getData('ltPropAnimation');
            ...
            _showPopover : function(){
                ...
                if(this.getData('ltPropShow')){
                    ...
                    if(result !== false){
                        ...
                        popoverElem[0].style.zIndex = window._lyteUiUtils.getZIndex();
...
LyteBetaPopoverComponent.register("lyte-beta-popover", {
    hash: "LyteBetaPopoverComponent_4",
