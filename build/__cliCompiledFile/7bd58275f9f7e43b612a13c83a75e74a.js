...
class LyteSearchComponent extends Component {
   constructor() {
      super();
      this._lyteUtilFunctions = [ "setValue", "focus", "blur", "select", "click" ];
   }
               ...
               if(! _children.childElementCount)
                   {
                     ...
                     if(query.target)
                       {
                         var scope = this.getScope( typeof query.scope == 'string' ? document[ window._lyteUiUtils.isWidget ? "querySelectorGlobal" : "querySelector" ]( query.scope.trim() ) : query.scope ),
   ...
   pressFunc(val, evt) {
        ...
        var scope = typeof query.scope == 'string' ? document[ window._lyteUiUtils.isWidget ? "querySelectorGlobal" : "querySelector" ]( query.scope.trim() ) : query.scope;
...
LyteSearchComponent._observedAttributes = [
   ...
   "ltPropDiacritic",
   "ltPropMaintainState",
   "ltPropPreventEmptyKeys",
   "ltPropMultipleSearch",
   "ltPropIgnoreChildren",
   "ltPropInput",
   "ltPropDontHideOnEmpty",
   "ltPropAria",
   "ltPropAriaAttributes",
   "timeout",
   "ltPropCheckFromParent"
];
...
LyteSearchComponent.register("lyte-search", {
   hash: "LyteSearchComponent_4",
