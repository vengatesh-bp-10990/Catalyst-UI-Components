...
class LyteWormholeComponent extends Component {
    ...
    appendContent() {
		var ret, 
		outlet = this.data.ltPropQuery ? document[  window._lyteUiUtils.isWidget ? "querySelectorGlobal" : 'querySelector' ]( this.data.ltPropQuery ) : document.body;
			...
			window._lyteUiUtils.popupStack.globalStack.forEach(function(ele,ind){
				if(ele.parentElement==utilObj.parentElement){
					if(ind<window._lyteUiUtils.popupStack.globalStack.length-1){
						changeFocus=false;
						window._lyteUiUtils.popupStack.globalStack[ind+1].focusedElement =	window._lyteUiUtils.popupStack.globalStack[ind].focusedElement;
					}
