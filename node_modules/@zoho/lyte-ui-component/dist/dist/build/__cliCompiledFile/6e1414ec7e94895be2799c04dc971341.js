...
ClientCompilerInternal.compileDynamicTemplate = function(actualComponent, templateToRender) {
    ...
    let gen = _LC.doCompile(templateToRenderAct.content, dynamicNodes.dynamicNodes, actualComponent.localName, actualComponent.constructor, undefined, actualComponent.component.constructor);
        ...
        if(compClassDef.activeInstances > 0) {
            ...
            if(this.getDynamicNodes) {
                ...
                if(returnVal.errors){
                    ...
                    compClassDef.__isRegistered = false;
                    if(registryClass.lazyRegister){
                        var lazyScheduler = registryClass.lazyScheduler;
                        if(lazyScheduler.currentTask && lazyScheduler.currentTask.id === registryClass.name+"$"+componentName){
                            lazyScheduler.currentTask.addEventListener("done", function(){
...
_LC.registerComponent = function(componentName, componentClass ,registryInstance){
    ...
    if(componentClass.__isRegistered){
