...
class LyteListviewFilterComponent extends Component {
    constructor() {
        super();
    }
    ...
    data(arg1) {
		return Object.assign(super.data({
			...
			ltPropRenderContent: prop('array', { default: [] }),
			ltPropFilteredContent: prop('array', { default: [] }),
			ltPropGroupedRow: prop('boolean'),
			ltPropCondition: prop('object', { default: {}, watch: true }),
			ltPropPlaceHolder : prop( 'string' , { default: 'Filters' } ),
			ltPropTriggerEvent : prop( 'boolean'),
			ltPropNavigator : prop( 'boolean' , { default : false } ),
			ltPropFilterContent: prop('array'),
			ltPropFormat : prop( 'string' ),
			ltPropNestedGrouping : prop( "boolean", { default :false } ),

			heading: prop('array', { default: [] }),
			inputPlaceHolder: prop('string', { default: "Filters" }),

			ltPropSearchContent: prop('array', { watch: true, default: [] }),
			renderContent: prop('array', { default: [] }),
			currentContent: prop('array', { default: [] }),
			currentFilterCount: prop('number', { default: 0 }),
			focusedElem: prop('object', { default: undefined }),
			showInput: prop('boolean', { default: true }),
			containsSortBy: prop('boolean', { default: false }),
			containsGroupBy: prop('boolean', { default: false }),
			containsContains : prop('boolean', { default : false } ),
			conditions: prop('object', { default: [] }),
			curDate: prop('string', { default: '' }),

			inputValue: prop('string', { default: "" }),
			hideInput: prop('boolean', { default: false }),
			...
			filterBy: prop('array', { default: ["= Equal to", "< Less than", "> Greater than", "= Not Equal to"] }),

			textFilter: prop('array', {
				default: [
			...
			multiselectFilter: prop('array', { default: [] }),
			noResultMessage: prop('boolean', { default: false }),
			headingsRemoved: prop('array' , { default : [] } ),
			ltPropPrevGroupIndex : prop( 'object' , { default : {} } ),
			headerMap : prop( 'object', { default : {} } ),
			renderCustom : prop( 'string', { default : "" } ),
			customValue : prop( 'string', { default : "" } )
            ...
            removeFilter: function (_this) {
                ...
                if (cur.name) {
                    if (cur.name == 'Sort by') {
                        ...
                        if (ret.columnIndex) {
                            objectUtils(this.getData('ltPropHeader')[ret.columnIndex].children[ret.rowIndex], 'add', "sortStatus", "");
                ...
                arrayUtils(this.data.ltPropSearchContent, 'removeAt', index, 1);
    ...
    onFilterBlur(_this, focus) {
		...
		if (_this.classList.contains("lyteInputFilterName")) {
			if (_this.type === "date" && _this.value) {
				...
				objectUtils(curContent,'add', 'conditionValue', _this.value);
				arrayUtils(searchContent, 'replaceAt', index, curContent);
			}else if( _this.classList.contains( 'lyteDateFilterElement' ) && customVal ){
				index = this.getIndex(_this);
				curContent = searchContent[index];
				objectUtils(curContent,'add', 'conditionValue', this.data.customValue);
    ...
    processSelection(filterIndex, event) {
		...
		if (isFilterInput || (((event && target.classList.contains('lyteInputFilterName'))) && !isFullySpecified)) {
			...
			arrayUtils(this.getData('ltPropSearchContent'), 'replaceAt', filterIndex, content);
			...
			if (currentHighlight) {
				...
				arrayUtils(this.getData('ltPropSearchContent'), 'push', { name: currentHighlight });
					...
					switch( __filterName ){
						...
						case 'Filter by': {
							...
							if( is_multiple_filter ){
								...
								if( __arr ){
									arrayUtils( __arr, 'push', condition );
								} else {
									objectUtils( conditions, 'add', heading, [ condition ] );
								}
							} else {
								objectUtils( conditions, 'add', heading, condition );
							}
			...
			filterContent.forEach(function (filter) {
				...
				if (isFullySpecified) {
					switch (filterName) {
						case 'Sort by':
							...
							objectUtils(_this.getData('ltPropHeader')[ret.columnIndex].children[ret.rowIndex], 'add', "sortStatus", className);
				...
				if (filterName === "Filter by" && condition) {
					...
					if (_this.data.ltPropMultipleFilter) {
						...
						arr.push(condition);
						objectUtils(conditions, 'add', heading, arr);
					} else {
						objectUtils(conditions, 'add', heading, condition);
					}
				}
    ...
    conditionValueClick(_this) {
		let index = parseInt(_this.parentElement.getAttribute('index')),
			...
			arrayUtils(content, 'replaceAt', index, cur_content);
    ...
    insertConditionList(filterName, content, index, target) {
		...
		arrayUtils(this.getData('ltPropSearchContent'), 'replaceAt', index, content);
...
LyteUiComponentComponentRegistry.registerHelper("shouldRenderInput", function (content) {
	return this.$node.component.isFilterFullySpecified(content);
});

LyteUiComponentComponentRegistry.registerHelper("getType", function (filter) {
	if ( filter.condition ) {
		return this.$node.component.getDataType(filter.column);
	}
});

export { LyteListviewFilterComponent };
