...
import {shouldIgnoreDisconnect, arrayUtils } from "../../node_modules/@slyte/component/index.js";
 ...
 */
class LyteExpresstableComponent extends Component {
    constructor() {
        super();
        this._lyteUtilFunctions = ["fixRowHeight", "scrollTo", "setHeaderWidth", "fixColumn", "setColumnWidth", "setVisibleFixedHeader", "reset"];
    }
    ...
    data(arg1) {
        return Object.assign(super.data({
            ...
            ltPropInfiniteScroll: prop('boolean', { 'default': false }),
            /** 
              * @componentProperty {boolean} ltPropYield=false
              * @version 3.1.0
              */
             ...
             * @version 3.1.0
             */
            ltPropMinWidth: prop('string', { 'default': '50px' }),
            /** 
             * @componentProperty {string} ltPropMaxWidth='1000px'
             * @version 3.1.0
             */
            ltPropMaxWidth: prop('string', { 'default': '1000px' }),
            /** 
              * @componentProperty {boolean} ltPropFixedTableScroll=false
              * @version 3.1.0
              */
            ltPropFixedTableScroll: prop('boolean', { 'default': false }),
            /** 
            ...
            ltPropRowViewPort: prop('boolean', { default: false }),
            fixedTableHeader: prop("array", { "default": [] }),
            fixedTableContent: prop("array", { "default": [] }),
            tableHeader: prop("array", { "default": [] }),
            tableContent: prop("array", { "default": [] }),
    ...
    init() {
        ...
        this._scrollWidth = window._lyteUiUtils.getScrollBarWidth();
        ...
        this._FF = /firefox/ig.test(window.navigator.userAgent);
        this._fixHeaderCount = 0
        this._tmpScrollTop = 0
        this._chrome = /Chrome/i.test(window.navigator.userAgent) && /Google Inc/i.test(window.navigator.vendor);
        this._isNegative = window._lyteUiUtils.isNegativeScroll()
    ...
    didConnect() {
        ...
        if (this.getData('ltPropNavigation') && !this.getData('ltPropStickyTable')) {
            ...
            window._lyteUiUtils.tableNavigation(this.$node.querySelector('.lyteExpTableWrapper'), options);
        } else if (this.getData('ltPropNavigation')) {
            window._lyteUiUtils.tableNavigation(this.$node.querySelector('.lyteExpOriginalTable'));
        }
        // this.$node.setHeaderWidth = function(){
        //     this.fixHeaderHeight();
        // }.bind( this );
    ...
    beforeNavigationFunc(tableCell) {
    ...
    afterNavigationFunc(tableCell, event) {
        this._oriwrap = this._oriwrap || this.$node.getElementsByClassName('lyteExpTableOrigTableInnerWrap')[0];
    ...
    setFixedTable() {
        ...
        for (var i = 0; i < tableHeader.length; i++) {
            if (tableHeader[i].fixed && tableHeader[i].fixed == "enable" && originalTableHeader[i].offsetWidth != 0) {
                arrayUtils(fixedTableHeader, 'push', tableHeader[i])
            }
        }
    ...
    setTableHeader() { // for setting tableHeader 
    ...
    fixHeaderHeight() { // setting min-width value for fixedTable and both fakeHeader 
    ...
    //     }
    // },
    setFixTableRowHeight(row) { //Fixing Row Height 
        var fDiv = this._colwrap || this.$node.getElementsByClassName('lyteExpTableFixedColWrapper')[0]
    ...
    setFixTableColumnWidth(col) { //Fixing Row Height 
    ...
    setVisibleFixHeader() { // To fix and unfix Columns
    ...
    scrollCheck(scrollTop, scrollLeft, ignore) {
    ...
    scrollF(scrollTop, scrollLeft, ignore) {
        this.scrollCheck(scrollTop, scrollLeft, ignore);
        // this.fixHeaderHeight();
    }

    verticalScroll(scrollLeft, flag) {
    ...
    setLeftDuringResize(offset, resizeComponent) {
    ...
    scrollFunc(event, scrollLeft, scrollTop, ignore) {
        if (this.getData('ltPropStickyTable')) {
            // if( this._FF ){
    ...
    toPrevent(scrollLeft, scrollWidth, offsetWidth, value) {
    ...
    getWheel(evt) {
        ...
        if (Math.abs(evt.deltaX || 0) >= Math.abs(evt.deltaY || 0)) {
            ...
            if (/edge|trident|msie/i.test(window.navigator.userAgent) && this._dir) {
                fact = -1;
            }
            return x * fact;
        }
    ...
    hideRows(bcr) {
        ...
        for (var i = 1; i < originalRows.length; i++) {
            ...
            if (_bcr.bottom < tValue) {
                ...
                arrayUtils(visible, 'push', originalRows.slice(i, i + 10));
                arrayUtils(topRows, 'push', originalRows.splice(i + 10));
                if (!isSticky) {
                    arrayUtils(visible, 'push', fakerows.slice(i, i + 10));
                    arrayUtils(topRows, 'push', fakerows.splice(i + 10));
                }
                break;
            } else {
                visible.push(row);
    ...
    returntrans(scrollLeft, offsetWidth, scrollWidth) {
        var value;
        if (this._isIE) {
    ...
    sortmousemove(ev, allow, pos) {
        ...
        if (closestTd && this.getMethods('onBeforeInterChange')) {
            ...
            if (interchangeprevent && newone) {
                arrayUtils(this._affectedIndex, 'removeAt', this._affectedIndex.indexOf(index));
            }
        }
        if (prevtable) {
    ...
    sortmouseup(evt) {
        ...
        if (this._moved) {
            ...
            if (this.getMethods('onBeforeDrop') && this.executeMethod('onBeforeDrop', evt, td, index, newindex, this.$node) == false) {
                failed = true;
                // if(td && this._fixedHeader){
                //     td.setAttribute('fixed','enable')
            ...
            if (!failed) {
                if (index != newindex) {
                    ...
                    var header = this.data.ltPropHeader,
                        current = arrayUtils(header, 'removeAt', index);
                    delete this._prevent;
                    arrayUtils(header, 'insertAt', newindex, current);
                }
    ...
    static observers(arg1) {
        return Object.assign(super.observers({
            ariaObserver: function (change) {
                window._lyteUiUtils.setAttribute(this.$node.querySelector('.lyteExpTableWrapper'), this.getData('ltPropAriaAttributes') || {}, {});
...
LyteExpresstableComponent._template = "<template tag-name=\"lyte-expresstable\"> <div class=\"lyteExpTableWrapper\" role=\"table\"> <template is=\"switch\" l-c=\"true\" _new=\"true\"><template case=\"{{ltPropFullYield}}\" is=\"case\" lc-id=\"lc_id_0\"> <div class=\"lyteExpTableOrigTableInnerWrap\" tabindex=\"{{ltPropTabIndex}}\" data-tabindex=\"{{ltPropDataTabIndex}}\" onscroll=\"{{action('scroll',event)}}\"> <lyte-yield class=\"lyteExpOriginalTable\" yield-name=\"fullYield\" lt-prop-header=\"{{tableHeader}}\" lt-prop-content=\"{{ltPropContent}}\"></lyte-yield> </div> </template><template default=\"\"><template is=\"switch\" l-c=\"true\" _new=\"true\"><template case=\"{{expHandlers(ltPropStickyTable,'!')}}\" is=\"case\" lc-id=\"lc_id_0\"> <template is=\"switch\" l-c=\"true\" _new=\"true\"><template case=\"{{expHandlers(ltPropHeaderLabelKey,'&amp;&amp;',tableHeader.length)}}\" is=\"case\" lc-id=\"lc_id_0\"> <div class=\"lyteExpTableFakeHeaderWrapper\"> <div class=\"lyteExpTableFixedColHeader\"> <template is=\"switch\" l-c=\"true\" _new=\"true\"><template case=\"{{expHandlers(ltPropYield,'==',false)}}\" is=\"case\" lc-id=\"lc_id_0\"> <lyte-exptable-thead class=\"fixedColumnTableHeader lyteExpTableHeaderGroup\" role=\"rowgroup\"> <lyte-exptable-tr class=\"fixedColumnTableRow lyteExpTableRow\" role=\"row\"> <template items=\"{{fixedTableHeader}}\" item=\"list\" index=\"indexVal\" is=\"for\" _new=\"true\"> <lyte-exptable-th class=\"fixedColumn lyteExpTableHead {{list.class}}\" id=\"{{list.id}}\" index=\"{{indexVal}}\" resize=\"{{list.resize}}\" fixed=\"{{list.fixed}}\" icon=\"{{list.icon}}\" role=\"columnheader\"> {{list[ltPropHeaderLabelKey]}} </lyte-exptable-th> </template> </lyte-exptable-tr> </lyte-exptable-thead> </template><template default=\"\"> <lyte-yield class=\"lyteExpTableFakeHeaderGroup lyteExpTableHeaderGroup\" yield-name=\"headerYield\" lt-prop-table-header=\"{{fixedTableHeader}}\"></lyte-yield> </template></template></div> <div class=\"lyteExpTableFakeColHeaderWrapper\"> <template is=\"switch\" l-c=\"true\" _new=\"true\"><template case=\"{{expHandlers(ltPropYield,'==',false)}}\" is=\"case\" lc-id=\"lc_id_0\"> <lyte-exptable-thead class=\"lyteExpTableFakeColHeader lyteExpTableHeaderGroup\" role=\"rowgroup\"> <lyte-exptable-tr class=\"originalColumnTableRow lyteExpTableRow\" role=\"row\"> <template items=\"{{tableHeader}}\" item=\"list\" index=\"indexVal\" is=\"for\" _new=\"true\"> <lyte-exptable-th id=\"{{list.id}}\" class=\"originalColumn lyteExpTableHead list.class\" index=\"{{indexVal}}\" resize=\"{{list.resize}}\" fixed=\"{{list.fixed}}\" icon=\"{{list.icon}}\" role=\"columnheader\"> {{list[ltPropHeaderLabelKey]}} </lyte-exptable-th> </template> </lyte-exptable-tr> </lyte-exptable-thead> </template><template default=\"\"> <lyte-yield class=\"lyteExpTableFakeColHeader lyteExpTableHeaderGroup\" yield-name=\"headerYield\" lt-prop-table-header=\"{{tableHeader}}\" role=\"rowgroup\"></lyte-yield> </template></template></div> </div> </template></template></template></template> <div class=\"lyteExpTableOrigTableWrapper \"> <template is=\"switch\" l-c=\"true\" _new=\"true\"><template case=\"{{expHandlers(ltPropStickyTable,'!')}}\" is=\"case\" lc-id=\"lc_id_0\"> <div class=\"lyteExpTableFixedColWrapper\" onscroll=\"{{action('fixedColScroll',event)}}\"> <lyte-exptable class=\"lyteExpTableFixedColTable\"> <template is=\"switch\" l-c=\"true\" _new=\"true\"><template case=\"{{expHandlers(ltPropYield,'==',false)}}\" is=\"case\" lc-id=\"lc_id_0\"> <lyte-exptable-thead class=\"fixedColumn lyteExpTableHeaderGroup\" role=\"rowgroup\"> <lyte-exptable-tr class=\"fixedColumn lyteExpTableRow\" role=\"row\"> <template items=\"{{fixedTableHeader}}\" item=\"list\" index=\"indexVal\" is=\"for\" _new=\"true\"> <lyte-exptable-th id=\"{{list.id}}\" class=\"fixedColumn lyteExpTableHead list.class\" index=\"{{indexVal}}\" resize=\"{{list.resize}}\" fixed=\"{{list.fixed}}\" icon=\"{{list.icon}}\" role=\"columnheader\"> {{unescape(list[ltPropHeaderLabelKey])}} </lyte-exptable-th> </template> </lyte-exptable-tr> </lyte-exptable-thead> <lyte-exptable-tbody class=\"fixedColumn lyteExpTableRowGroup\" role=\"rowgroup\"> <template is=\"for\" items=\"{{ltPropContent}}\" item=\"list\" index=\"indexVal\"> <lyte-exptable-tr id=\"{{list.id}}\" class=\"fixedColumn lyteExpTableRow {{list.class}}\" role=\"row\" aria-rowindex=\"{{indexVal}}\"> <template is=\"for\" items=\"{{fixedTableHeader}}\" item=\"header\" index=\"index\"> <lyte-exptable-td class=\"fixedColumn lyteExpTableCell\" role=\"cell\" aria-colindex=\"{{index}}\">{{unescape(lyteUiGetValue(list,header[ltPropBodyLabelKey]))}}</lyte-exptable-td> </template> </lyte-exptable-tr> </template> </lyte-exptable-tbody> </template><template default=\"\"> <lyte-yield class=\"lyteExpTableHeaderGroup\" yield-name=\"headerYield\" lt-prop-table-header=\"{{fixedTableHeader}}\" role=\"rowgroup\"></lyte-yield> <lyte-yield class=\"lyteExpTableRowGroup\" yield-name=\"contentYield\" lt-prop-table-header=\"{{fixedTableHeader}}\" role=\"rowgroup\"> </lyte-yield> </template></template></lyte-exptable> </div> </template></template><template is=\"switch\" l-c=\"true\" _new=\"true\"><template case=\"{{expHandlers(ltPropHeaderLabelKey,'&amp;&amp;',tableHeader.length)}}\" is=\"case\" lc-id=\"lc_id_0\"> <div class=\"lyteExpTableOrigTableInnerWrap\" tabindex=\"{{ltPropTabIndex}}\" data-tabindex=\"{{ltPropDataTabIndex}}\" onscroll=\"{{action('scroll',event)}}\"> <lyte-exptable class=\"lyteExpOriginalTable\"> <template is=\"switch\" l-c=\"true\" _new=\"true\"><template case=\"{{expHandlers(ltPropYield,'==',false)}}\" is=\"case\" lc-id=\"lc_id_0\"> <lyte-exptable-thead class=\"originalColumn lyteExpTableHeaderGroup\" role=\"rowgroup\"> <lyte-exptable-tr class=\"originalColumn lyteExpTableRow\" role=\"row\"> <template items=\"{{tableHeader}}\" item=\"list\" index=\"indexVal\" is=\"for\" _new=\"true\"> <lyte-exptable-th id=\"{{list.id}}\" class=\"originalColumn lyteExpTableHead list.class\" index=\"{{indexVal}}\" resize=\"{{list.resize}}\" fixed=\"{{list.fixed}}\" icon=\"{{list.icon}}\" role=\"columnheader\"> {{list[ltPropHeaderLabelKey]}} </lyte-exptable-th> </template> </lyte-exptable-tr> </lyte-exptable-thead> <lyte-exptable-tbody class=\"originalColumn lyteExpTableRowGroup\" role=\"rowgroup\"> <template is=\"for\" items=\"{{ltPropContent}}\" item=\"list\" index=\"indexVal\"> <lyte-exptable-tr id=\"{{list.id}}\" class=\"originalColumn lyteExpTableRow {{list.class}}\" role=\"row\" aria-rowindex=\"{{indexVal}}\"> <template is=\"for\" items=\"{{tableHeader}}\" item=\"header\" index=\"index\"> <lyte-exptable-td class=\"originalColumn lyteExpTableCell\" role=\"cell\" aria-colindex=\"{{index}}\">{{unescape(lyteUiGetValue(list,header[ltPropBodyLabelKey]))}}</lyte-exptable-td> </template> </lyte-exptable-tr> </template> </lyte-exptable-tbody> </template><template default=\"\"> <lyte-yield class=\"lyteExpTableHeaderGroup\" yield-name=\"headerYield\" lt-prop-table-header=\"{{tableHeader}}\" role=\"rowgroup\"></lyte-yield> <lyte-yield class=\"lyteExpTableRowGroup\" yield-name=\"contentYield\" lt-prop-table-header=\"{{tableHeader}}\" role=\"rowgroup\"></lyte-yield> </template></template></lyte-exptable> </div> </template></template></div> </template></template></div> </template>";;
LyteExpresstableComponent._dynamicNodes = [{"t":"s","p":[1,1],"c":{"lc_id_0":{"dN":[{"t":"a","p":[1],"cn":"lc_id_0"},{"t":"a","p":[1,1],"cn":"lc_id_0"},{"t":"i","p":[1,1],"in":0,"cn":"lc_id_0"}],"cdp":{"t":"a","p":[0]},"dcn":true}},"d":{"dN":[{"t":"s","p":[0],"c":{"lc_id_0":{"dN":[{"t":"s","p":[1],"c":{"lc_id_0":{"dN":[{"t":"s","p":[1,1,1],"c":{"lc_id_0":{"dN":[{"t":"a","p":[1,1,1],"cn":"lc_id_0"},{"t":"f","p":[1,1,1],"dN":[{"t":"a","p":[1]},{"t":"tX","p":[1,1]},{"t":"cD","p":[1],"in":0}],"dc":[0],"hc":true,"trans":true,"in":2,"sibl":[1],"cn":"lc_id_0"},{"t":"cD","p":[1,1],"in":1,"sibl":[0],"cn":"lc_id_0"},{"t":"cD","p":[1],"in":0,"cn":"lc_id_0"}],"cdp":{"t":"a","p":[0]},"dcn":true}},"d":{"dN":[{"t":"a","p":[1],"cn":"default"},{"t":"i","p":[1],"in":0,"cn":"default"}]},"dc":{"lc_id_0":{"dc":[2,1,0],"hc":true,"trans":true},"default":{"dc":[0],"hc":true,"trans":true}},"hd":true,"co":["lc_id_0"],"hc":true,"trans":true,"in":1,"sibl":[0],"cn":"lc_id_0"},{"t":"s","p":[1,3,1],"c":{"lc_id_0":{"dN":[{"t":"a","p":[1,1,1],"cn":"lc_id_0"},{"t":"f","p":[1,1,1],"dN":[{"t":"a","p":[1]},{"t":"tX","p":[1,1]},{"t":"cD","p":[1],"in":0}],"dc":[0],"hc":true,"trans":true,"in":2,"sibl":[1],"cn":"lc_id_0"},{"t":"cD","p":[1,1],"in":1,"sibl":[0],"cn":"lc_id_0"},{"t":"cD","p":[1],"in":0,"cn":"lc_id_0"}],"cdp":{"t":"a","p":[0]},"dcn":true}},"d":{"dN":[{"t":"a","p":[1],"cn":"default"},{"t":"i","p":[1],"in":0,"cn":"default"}]},"dc":{"lc_id_0":{"dc":[2,1,0],"hc":true,"trans":true},"default":{"dc":[0],"hc":true,"trans":true}},"hd":true,"co":["lc_id_0"],"hc":true,"trans":true,"in":0,"cn":"lc_id_0"}],"cdp":{"t":"a","p":[0]},"dcn":true}},"d":{},"dc":{"lc_id_0":{"dc":[1,0],"hc":true,"trans":true}},"hd":true,"co":["lc_id_0"],"hc":true,"trans":true,"in":0,"cn":"lc_id_0"}],"cdp":{"t":"a","p":[0]},"dcn":true}},"d":{},"dc":{"lc_id_0":{"dc":[0],"hc":true,"trans":true}},"hd":true,"co":["lc_id_0"],"hc":true,"trans":true,"in":2,"sibl":[1],"cn":"default"},{"t":"s","p":[2,1],"c":{"lc_id_0":{"dN":[{"t":"a","p":[1],"cn":"lc_id_0"},{"t":"s","p":[1,1,1],"c":{"lc_id_0":{"dN":[{"t":"a","p":[1,1,1],"cn":"lc_id_0"},{"t":"f","p":[1,1,1],"dN":[{"t":"a","p":[1]},{"t":"tX","p":[1,1]},{"t":"cD","p":[1],"in":0}],"dc":[0],"hc":true,"trans":true,"in":4,"sibl":[3],"cn":"lc_id_0"},{"t":"cD","p":[1,1],"in":3,"sibl":[2],"cn":"lc_id_0"},{"t":"cD","p":[1],"in":2,"sibl":[1],"cn":"lc_id_0"},{"t":"a","p":[3,1],"cn":"lc_id_0"},{"t":"f","p":[3,1],"dN":[{"t":"a","p":[1]},{"t":"a","p":[1,1]},{"t":"f","p":[1,1],"dN":[{"t":"a","p":[1]},{"t":"tX","p":[1,0]},{"t":"cD","p":[1],"in":0}],"dc":[0],"hc":true,"trans":true,"in":1,"sibl":[0]},{"t":"cD","p":[1],"in":0}],"dc":[1,0],"hc":true,"trans":true,"in":1,"sibl":[0],"cn":"lc_id_0"},{"t":"cD","p":[3],"in":0,"cn":"lc_id_0"}],"cdp":{"t":"a","p":[0]},"dcn":true}},"d":{"dN":[{"t":"a","p":[1],"cn":"default"},{"t":"i","p":[1],"in":1,"sibl":[0],"cn":"default"},{"t":"a","p":[3],"cn":"default"},{"t":"i","p":[3],"in":0,"cn":"default"}]},"dc":{"lc_id_0":{"dc":[4,3,2,1,0],"hc":true,"trans":true},"default":{"dc":[1,0],"hc":true,"trans":true}},"hd":true,"co":["lc_id_0"],"hc":true,"trans":true,"in":1,"sibl":[0],"cn":"lc_id_0"},{"t":"cD","p":[1,1],"in":0,"cn":"lc_id_0"}],"cdp":{"t":"a","p":[0]},"dcn":true}},"d":{},"dc":{"lc_id_0":{"dc":[1,0],"hc":true,"trans":true}},"hd":true,"co":["lc_id_0"],"hc":true,"trans":true,"in":1,"sibl":[0],"cn":"default"},{"t":"s","p":[2,2],"c":{"lc_id_0":{"dN":[{"t":"a","p":[1],"cn":"lc_id_0"},{"t":"s","p":[1,1,1],"c":{"lc_id_0":{"dN":[{"t":"a","p":[1,1,1],"cn":"lc_id_0"},{"t":"f","p":[1,1,1],"dN":[{"t":"a","p":[1]},{"t":"tX","p":[1,1]},{"t":"cD","p":[1],"in":0}],"dc":[0],"hc":true,"trans":true,"in":4,"sibl":[3],"cn":"lc_id_0"},{"t":"cD","p":[1,1],"in":3,"sibl":[2],"cn":"lc_id_0"},{"t":"cD","p":[1],"in":2,"sibl":[1],"cn":"lc_id_0"},{"t":"a","p":[3,1],"cn":"lc_id_0"},{"t":"f","p":[3,1],"dN":[{"t":"a","p":[1]},{"t":"a","p":[1,1]},{"t":"f","p":[1,1],"dN":[{"t":"a","p":[1]},{"t":"tX","p":[1,0]},{"t":"cD","p":[1],"in":0}],"dc":[0],"hc":true,"trans":true,"in":1,"sibl":[0]},{"t":"cD","p":[1],"in":0}],"dc":[1,0],"hc":true,"trans":true,"in":1,"sibl":[0],"cn":"lc_id_0"},{"t":"cD","p":[3],"in":0,"cn":"lc_id_0"}],"cdp":{"t":"a","p":[0]},"dcn":true}},"d":{"dN":[{"t":"a","p":[1],"cn":"default"},{"t":"i","p":[1],"in":1,"sibl":[0],"cn":"default"},{"t":"a","p":[3],"cn":"default"},{"t":"i","p":[3],"in":0,"cn":"default"}]},"dc":{"lc_id_0":{"dc":[4,3,2,1,0],"hc":true,"trans":true},"default":{"dc":[1,0],"hc":true,"trans":true}},"hd":true,"co":["lc_id_0"],"hc":true,"trans":true,"in":1,"sibl":[0],"cn":"lc_id_0"},{"t":"cD","p":[1,1],"in":0,"cn":"lc_id_0"}],"cdp":{"t":"a","p":[0]},"dcn":true}},"d":{},"dc":{"lc_id_0":{"dc":[1,0],"hc":true,"trans":true}},"hd":true,"co":["lc_id_0"],"hc":true,"trans":true,"in":0,"cn":"default"}]},"dc":{"lc_id_0":{"dc":[0],"hc":true,"trans":true},"default":{"dc":[2,1,0],"hc":true,"trans":true}},"hd":true,"co":["lc_id_0"],"hc":true,"trans":true,"in":0},{"type":"dc","trans":true,"hc":true,"p":[0]}];;

LyteExpresstableComponent._observedAttributes = [
    "ltPropHeader",
    "ltPropContent",
...
if (!window._lyteUiUtils.registeredCustomElements['lyte-exptable-th']) {
    window._lyteUiUtils.registeredCustomElements['lyte-exptable-th'] = true;

    class LyteExptableThCustomElements extends LyteUiComponentComponentRegistry.CustomElements {
        lookups() {
            return [{
        ...
        attributeChangedCallback(attr, oldVal, newVal) {
            ...
            if (attr == 'fixed') {
                var scrollingDiv = lyteNode;
                while (scrollingDiv && scrollingDiv.tagName != 'DIV') {
                    scrollingDiv = scrollingDiv.parentNode;
                }
                if (newVal == 'enable') {
                    lyteNode.classList.add('lyteFixedColumn');
                } else {
                    lyteNode.classList.remove('lyteFixedColumn');
                }
                ...
                if (newVal == 'enable') {
                    ...
                    lyteNode.appendChild(tabHead);
                } else {
                    var tabHead = lyteNode.getElementsByTagName('lyte-exptablehead-resize')[0];
                    if (tabHead) {
                        lyteNode.removeChild(tabHead);
                    }
                }
            }
        }

        checkIntersection(arg) {
            var lyteNode = this.$node;
            var table = $L(lyteNode).closest('lyte-expresstable').get(0);
            if (table.getData('ltPropStickyTable')) {
                if (arg && !lyteNode._horizontalIntersectionDiv) {
                    this.createIntersection(table);
                } else if (!arg && lyteNode._horizontalIntersectionDiv) {
                    var intersection = lyteNode._horizontalIntersectionDiv;
                    this.removeIntersection(intersection, table);
                    table.component.removeSticky(lyteNode);
                    table.component.removeFixedClass(lyteNode);
                }
            }
        }
        ...
        createIntersection(table) {
            ...
            lyteNode._horizontalIntersectionDiv = div;
            div._cell = lyteNode;
        ...
        removeIntersection(intersection, table) {
            ...
            delete lyteNode._horizontalIntersectionDiv;
        }
    ...
    LyteExptableThCustomElements.options = {clone : {allCallbacks : false}};
    LyteExptableThCustomElements.register("lyte-exptable-th");
}

if (!window._lyteUiUtils.registeredCustomElements['lyte-exptable-tr']) {
    window._lyteUiUtils.registeredCustomElements['lyte-exptable-tr'] = true;

    class LyteExptableTrCustomElements extends LyteUiComponentComponentRegistry.CustomElements {
        lookups() {
            return [{
                component: LyteUiComponentComponentRegistry
            }];
        ...
        connectedCallback() {
            ...
            var query = "lyte-expresstable",
                table = lyteNode.closest ? lyteNode.closest(query) : $L(lyteNode).closest(query).get(0);

            if (table && table.ltProp('role') && table.ltProp('yield')) {
                table.component.update_aria(lyteNode);
            }
        }
    ...
    LyteExptableTrCustomElements.options = {clone : {allCallbacks : false}};

    LyteExptableTrCustomElements.register('lyte-exptable-tr');
}
...
*   </lyte-expresstable>  
*/
export { LyteExpresstableComponent };

LyteExpresstableComponent.register("lyte-expresstable", {
    hash: "LyteExpresstableComponent_2",
