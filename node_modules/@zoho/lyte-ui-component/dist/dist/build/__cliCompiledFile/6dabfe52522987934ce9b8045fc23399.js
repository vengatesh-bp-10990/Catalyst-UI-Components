...
window.addEventListener('click' , function(eve){
	if(window._lyteUiUtils.popupStack){
		if(window._lyteUiUtils.popupStack.globalStack && window._lyteUiUtils.popupStack.betaModalStack &&window._lyteUiUtils.popupStack.betaModalStack.length > 0){
			var lastPopup = window._lyteUiUtils.popupStack.globalStack[ window._lyteUiUtils.popupStack.globalStack.length - 1 ]
			if(window._lyteUiUtils.popupStack.globalStack[window._lyteUiUtils.popupStack.globalStack.length-1].parentElement === window._lyteUiUtils.popupStack.betaModalStack[window._lyteUiUtils.popupStack.betaModalStack.length-1].parentElement){
...
window.addEventListener('resize' , function(eve){
	if(window._lyteUiUtils.popupStack){
		if(window._lyteUiUtils.popupStack.globalStack && window._lyteUiUtils.popupStack.betaModalStack.length > 0){
			var modalArr = window._lyteUiUtils.popupStack.betaModalStack
...
class LyteBetaModalComponent extends Component {
    ...
    data(arg1) {
		return Object.assign(super.data({
             ...
             */
		     ...
		     "ltPropResizeModal": prop('boolean', { default: false }),
			 
			 "ltPropResizerMinWidth": prop('number', { default: 30 }),
			 
			 "ltPropResizerMaxWidth": prop('number', { default: window.innerWidth }),
			 
			 "ltPropResizerMinHeight": prop('number', { default: 30 }),

			 "ltPropResizerMaxHeight": prop('number', { default: window.innerHeight }),
			 ...
			 initialDragMouseDown : prop('object' , {
				default : {
					clientX : 0,
					clientY : 0,
					height : 0,
			...
			activeResizer: prop('string', {
				 default: ""
			}),
			returnedFalse : prop('boolean' , {
				default : false
			}),
			oppositeSidePosition: prop('string', {default: ""}),
			ignoreOffsetObserver: prop("boolean", {default: false})
		}), arg1);		
	}

    init() {
		var _this = this;
		if(!window._lyteUiUtils.lyteModalKeyDown){
			window._lyteUiUtils.lyteModalKeyDown = true
			document.addEventListener('keydown' , function(evt){
				var modalArr = []
				var popstack = []
				if(window._lyteUiUtils.popupStack && window._lyteUiUtils.popupStack.betaModalStack){
					modalArr = window._lyteUiUtils.popupStack.betaModalStack
				}
		...
		if(this.childComp && this.getData('ltPropShow')){
			...
			if(freeze[0]){
				if(this.getData('ltPropOverlayClose')){
					if(!window._lyteUiUtils.modalBodyClick){
						freeze[0].addEventListener('click' , this.closeOnBodyClick)
						window._lyteUiUtils.modalBodyClick = true	
					}
				} else {
					freeze[0].removeEventListener('click' , this.closeOnBodyClick)
					window._lyteUiUtils.modalBodyClick = false	
				}
			}
		}
	}
			...
			if(window._lyteUiUtils.popupStack){
				...
				this.closeModalTransition = setTimeout(function() {
					if($L('.lyteModalCurrentlyClosing').closest('lyte-wormhole')[0]){
						if(window._lyteUiUtils.popupStack && window._lyteUiUtils.popupStack.betaModalStack.length<1){
						...
						window._lyteUiUtils.dispatchEvent('lyteModalClosed' , _this.actualModalDiv)
    ...
    closeOtherModals() {
		if(window._lyteUiUtils.popupStack && window._lyteUiUtils.popupStack.betaModalStack && window._lyteUiUtils.popupStack.betaModalStack.length > 0){
			var prevIndex = window._lyteUiUtils.popupStack.betaModalStack.length-1
			if(!window._lyteUiUtils.popupStack.betaModalStack[prevIndex].parentElement.getData('ltPropAllowMultiple')){
				window._lyteUiUtils.popupStack.betaModalStack[prevIndex].parentElement.setData('ltPropShow' , false);
			}
		}
	}
    ...
    removePopoverBodyClick() {
		var popupstack = window._lyteUiUtils.popupStack.globalStack;
    ...
    dragMouseDown(eve) {
		if($L(eve.target).hasClass('lyteModalResizeNode')){
			...
			objectUtils(_this.getData('initialDragMouseDown') , 'add' , 'clientX' , eve.clientX)
			objectUtils(_this.getData('initialDragMouseDown') , 'add' , 'clientY' , eve.clientY)

			objectUtils(_this.getData('initialDragMouseDown') , 'add' , 'height' , modalElement.getBoundingClientRect().height)
			objectUtils(_this.getData('initialDragMouseDown') , 'add' , 'width' , modalElement.getBoundingClientRect().width)
			

			objectUtils(_this.getData('initialDragMouseDown') , 'add' , 'top' , innerModalElementRect.top)
			objectUtils(_this.getData('initialDragMouseDown') , 'add' , 'left' , innerModalElementRect.left)
			objectUtils(_this.getData('initialDragMouseDown') , 'add' , 'right' , innerModalElementRect.right)
			objectUtils(_this.getData('initialDragMouseDown') , 'add' , 'bottom' , innerModalElementRect.bottom)
        ...
        return Object.assign(super.observers({
            ...
            _showModal : function(){
                ...
                if(result !== false){
                    ...
                    var modalElem = $L(this.childComp).find('.lyteModalWrapper');
                    modalElem[0].style.zIndex = window._lyteUiUtils.getZIndex();
                    var modalInnerElem = modalElem.find('.lyteModalElement');
                    ...
                    if(!this.getData('ltPropOverlapModal')) {
                        var previousOpenedModal = window._lyteUiUtils.popupStack.betaModalStack[window._lyteUiUtils.popupStack.betaModalStack.length - 2].parentElement;
                    ...
                    setTimeout(function(){
                        if(!window._lyteUiUtils.modalBodyClick || _this.getData('ltPropReRenderModal')){
...
LyteBetaModalComponent._observedAttributes = [
    ...
    "ltPropFocusOnClose",
    "ltPropResizeModal",
    "ltPropResizerMinWidth",
    "ltPropResizerMaxWidth",
    "ltPropResizerMinHeight",
    "ltPropResizerMaxHeight",
    "showFreeze",
    "initialModalDim",
    "initialMouseDown",
    "initialDragMouseDown",
    "initialComputedModalDim",
    "closeDurationTimeOut",
    "reRenderModal",
    "popoverclosevaluechanged",
    "activeResizer",
    "returnedFalse",
    "oppositeSidePosition",
    "ignoreOffsetObserver"
];

export { LyteBetaModalComponent };

LyteBetaModalComponent.register("lyte-beta-modal", {
    hash: "LyteBetaModalComponent_2",
