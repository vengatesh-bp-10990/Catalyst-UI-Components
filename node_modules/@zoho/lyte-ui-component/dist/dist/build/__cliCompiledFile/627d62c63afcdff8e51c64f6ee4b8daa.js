...
import { appendChild } from "../../../node_modules/@slyte/component/index.js";
...
class LyteSchedulerEventTagComponent extends Component {
    ...
    isinBetween(curr_date, divPostion, endDate, scheduler) {
        var eventManipulator = new window._lyteUiUtils.eventManipulator( scheduler.getData('ltPropFormat') );
            ...
            if(eventData.allDayEvent ){
                ...
                if(startTimeDiv){
                    ...
                    if(!parent_div.contains(_this.$node)){
                        appendChild( parent_div, _this.$node);
                    }
                    ...
                    if( (width + eventLeft)  > (schedulerRight) ){
                        ...
                        if(overflow){
                            if(overflow.scrollHeight > overflow.clientHeight){
                                scrollBar = window._lyteUiUtils.getScrollBarWidth() ;
                            }
                        }
                        width =   ( schedulerRight -  paddingRight - eventLeft) - scrollBar ;
                    }
                }else{
                    width = (datetd_width) - 2 * paddingRight;
                }
    ...
    findIntersect(interset, event_div, date_div, index) {
    ...
    setWidthForTimeLine(parent, scheduler, top, height) {
        var view = scheduler.getData('ltPropSchedulerView');
        ...
        const parentWidth = parentElement.offsetWidth - padding;
        ...
        if (prevElem && col > 1) {
            ...
            if(scheduler.getData('ltPropWeekEventOverlap') && view == 'week'){
                if( scheduler.getData('ltPropWeekEventOverlap') && (prevElem.offsetTop + perv_intersect.offsetHeight) < parseFloat(top)){
                    if(isRTL){
                        right = ( prevElem.offsetParent.getBoundingClientRect().right - prevElem.getBoundingClientRect().right ) + (prevElem.offsetWidth * 0.20);
                        totalWidth -= (( prevElem.offsetParent.getBoundingClientRect().right - prevElem.getBoundingClientRect().right ) + (prevElem.offsetWidth * 0.20));
                    }else{
                    ...
                    if(isRTL){
                        ...
                        totalWidth -= (( prevElem.offsetParent.getBoundingClientRect().right - prevElem.getBoundingClientRect().right ) + (prevElem.offsetWidth * 0.5));
                    }else{
                ...
                if(isRTL){
                    right = (col - 1) * minWidth + (!(scheduler.getData('ltPropWeekEventOverlap') && view == 'week') ? gapBetweenEvents : 0);
                }else{
                    left = (col - 1) * minWidth + (!(scheduler.getData('ltPropWeekEventOverlap') && view == 'week') ? gapBetweenEvents : 0);
                }
                if(intersect && intersect.length > 0){
                    var ActualWidth = (totalWidth - left)/Math.min(intersect.length+1,depthArrayCount);
                }else{
...
LyteSchedulerEventTagComponent._template = "<template tag-name=\"lyte-scheduler-event-tag\"> <lyte-yield yield-name=\"scheduler-event-badge\"> </lyte-yield> </template>";;
LyteSchedulerEventTagComponent._dynamicNodes = [{"t":"i","p":[1],"in":0},{"type":"dc","trans":true,"hc":true,"p":[0]}];;
LyteSchedulerEventTagComponent._observedAttributes = ["ltPropEvent"];
export { LyteSchedulerEventTagComponent };

LyteSchedulerEventTagComponent.register("lyte-scheduler-event-tag", {
    hash: "LyteSchedulerEventTagComponent_2",
