...
class LyteGradientSliderComponent extends Component {
    constructor() {
        super();
        this._lyteUtilFunctions = [ "insertHandler", "removeHandler", "updateHandler", "getHandler", "focus" ];
    }
    ...
    init() {
		...
		this.$node.insertHandler = function( index, obj ){
			...
			arrayUtils( stops, "insertAt", index, obj );
			this.update_bg();
    ...
    update_value(offset, index) {
		...
		objectUtils( cur, "add", "offset", offset / 100 );
    ...
    update_order(cur) {
		...
		if( old_index != new_index ){
			arrayUtils( arr, 'removeAt', old_index );
			arrayUtils( arr, 'insertAt', new_index, cur );
		}
		this.update_bg();
    ...
    get_rtl() {
		return window._lyteUiUtils.getRTL() == true;
	}
    ...
    delete_handler(elem, evt) {
		...
		arrayUtils( stop, "removeAt", index );
		this.update_bg();
...
LyteGradientSliderComponent._template = "<template tag-name=\"lyte-gradient-slider\"> <template is=\"switch\" l-c=\"true\" _new=\"true\"><template case=\"{{ltPropControls}}\" is=\"case\" lc-id=\"lc_id_0\"><div class=\"lyteGradientControls\"> <span class=\"lyteGradientInsert\" onclick=\"{{action('insert',event)}}\"></span> <span class=\"lyteGradientRemove\" onclick=\"{{action('remove',event)}}\"></span> </div></template></template><div class=\"lyteGradientFillContainer\" style=\"background-image: {{ltPropBackgroundImage}};\"> <template items=\"{{ltPropStop}}\" item=\"item\" index=\"index\" is=\"for\" _new=\"true\"><span onfocus=\"{{action('focus',this)}}\" class=\"lyteGradientHandler\" tabindex=\"{{ltPropTabIndex}}\" data-index=\"{{index}}\" onmousedown=\"{{action('mousedown',event,item)}}\" ontouchstart=\"{{action('mousedown',event,item)}}\" onkeydown=\"{{action('keydown',event,item)}}\" aria-valuemin=\"0\" aria-valuemax=\"1\" aria-valuenow=\"{{item.offset}}\" aria-orientation=\"horizontal\" aria-valuetext=\"{{item.color}} {{item.offset}}\" style=\"{{lyteGradientSliderHandlerPosition(item.color,item.offset)}}\"></span></template> </div> </template>";;
LyteGradientSliderComponent._dynamicNodes = [{"t":"s","p":[1],"c":{"lc_id_0":{"dN":[{"t":"a","p":[0,1],"cn":"lc_id_0"},{"t":"a","p":[0,3],"cn":"lc_id_0"}],"cdp":{"t":"a","p":[0]},"dcn":true}},"d":{},"dc":{"lc_id_0":{}},"hd":true,"co":["lc_id_0"],"sibl":[0]},{"t":"a","p":[2],"a":{"style":{"name":"style","helperInfo":{"name":"concat","args":["'background-image: '","ltPropBackgroundImage","';'"]}}}},{"t":"a","p":[2,1]},{"t":"f","p":[2,1],"dN":[{"t":"a","p":[0],"a":{"style":{"name":"style","helperInfo":{"name":"lyteGradientSliderHandlerPosition","args":["item.color","item.offset"]}}}}]}];;

LyteGradientSliderComponent._observedAttributes = [
    "ltPropControls",
    "ltPropMinCount",
    "ltPropMaxCount",
    "ltPropIncrement",
...
export { LyteGradientSliderComponent };

LyteGradientSliderComponent.register("lyte-gradient-slider", {
    hash: "LyteGradientSliderComponent_2",
