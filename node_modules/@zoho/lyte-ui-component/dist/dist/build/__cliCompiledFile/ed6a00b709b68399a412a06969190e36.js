...
import { Component, arrayUtils, objectUtils } from "../../node_modules/@slyte/component/index.js";
 ...
 */
class LyteOrgTreeComponent extends Component {
    constructor() {
        super();
    }
    ...
    data(arg1) {
		 ...
		 * @default 2
		 */
			ltPropMinimumExpanded: prop('number', {
				default: 2
			}),
			/**
			 ...
			 * @default 5
			 */
			ltPropInitialRenderCount: prop('number', {
				default: 5
			}),
			/**
			 ...
			 * @version 1.0.0
			 */
			ltPropTargetKey: prop('string'),
			/**
			 * @componentProperty {string} ltPropChildrenCountKey
			 * @version 1.0.0
			 */
			ltPropChildrenCountKey: prop('string'),
			/**
			 * @componentProperty {number} ltPropDragThreshold
			 * @version 1.0.0
			 * @default 10
			 */
			ltPropDragThreshold: prop('number', {
				default: 10
			}),
			/**
			 ...
			 * @default []
			 */
			lyteOrgTreeMainArray: prop('array', {
				default: []
			}),
			/**
			 ...
			 * @default []
			 */
			lyteOrgTreePopulateArray: prop('array', {
				default: []
			}),
			/**
			 ...
			 */
			initialElemWrapDim: prop('object', {
				default: {
					width: 0,
					height: 0
				}
		 ...
		 * @version 1.0.0
		 */
			svgWrapHeight: prop('number'),
			/**
		 * @componentProperty {number} svgWrapWidth
		 * @version 1.0.0
		 */
			svgWrapWidth: prop('number')
		}), arg1);
	}
    ...
    didConnect() {
		...
		if (this.getData('ltPropTarget') !== "0") {
			...
			arrayUtils(this.getData('lyteOrgTreeMainArray'), 'push', [this.getData('ltPropData')])
			arrayUtils(this.getData('lyteOrgTreePopulateArray'), 'push', [this.getData('ltPropData')])
		}
		...
		objectUtils(this.getData('initialElemWrapDim'), 'add', 'width', dim.width)
		objectUtils(this.getData('initialElemWrapDim'), 'add', 'height', dim.height)
	}
        ...
        return Object.assign(super.actions({
            startDrag: function (th) {
                ...
                if (!$L(th).closest('.lyteOrgTreeElementsWrap').hasClass('lyteOrgTreeActiveWrap')) {
                    ...
                    objectUtils(this.getData('initialDimension'), 'add', 'left', holdElem.getBoundingClientRect().left)
                    objectUtils(this.getData('initialDimension'), 'add', 'top', holdElem.getBoundingClientRect().top)
                    objectUtils(this.getData('initialDimension'), 'add', 'prevClientX', event.clientX)
                    objectUtils(this.getData('initialDimension'), 'add', 'prevClientY', event.clientY)
            ...
            openChildLevel: function (th, item, level, eve) {
                ...
                if (level + 1 <= this.getData('lyteOrgTreeMainArray').length) {
                    ...
                    arrayUtils(this.getData('lyteOrgTreeMainArray'), 'splice', level + 1, (this.getData('lyteOrgTreeMainArray').length - (level + 1)));
                    arrayUtils(this.getData('lyteOrgTreePopulateArray'), 'splice', level + 1, (this.getData('lyteOrgTreePopulateArray').length - (level + 1)));
                ...
                if (item[this.getData('ltPropChildrenKey')] && (item[this.getData('ltPropChildrenKey')].length > 0)) {
                    ...
                    var minPushArr = []
                    pushArr = loopArr
                    arrayUtils(this.getData('lyteOrgTreeMainArray'), 'push', [pushArr])
                    ...
                    arrayUtils(this.getData('lyteOrgTreePopulateArray'), 'push', [minPushArr]);
            ...
            loadMoreData: function (level) {
                ...
                if (toPushArr.length < 1) {
                    if (ret) {
                        arrayUtils(currentArr, 'push', ret)
                    }
                } else {
                    arrayUtils(currentArr, 'push', toPushArr)
                }
            },
    ...
    appendScrollEvent(level) {
		...
		$L($L('.lyteOrgTreeColumn')[level]).lazyRender({
			...
			onEnd: function () {
				...
				if (ret) {
					arrayUtils(this.entireData, 'push', ret)
				}
			}
		})
	}
    ...
    getConnectorCoordinates(leftColumn, rightColumn, currentColumn) {
    ...
    connectorsScrollHandling(leftColumn, rightColumn, currentColumn, eve) {
    ...
    removeMoveEvent(event) {
		...
		if ($L(event.target).closest('.lyteOrgTreeElementsWrap').hasClass('lyteOrgTreeActiveWrap')) {
			if (mainArr[dropIndexes.column][dropIndexes.row][this.component.getData('ltPropChildrenKey')]) {
				arrayUtils(mainArr[dropIndexes.column][dropIndexes.row][this.component.getData('ltPropChildrenKey')], 'insertAt', popArr[dropIndexes.column + 1].length, [elemData])
				arrayUtils(popArr[dropIndexes.column + 1], 'push', [elemData])
			} else {
				objectUtils(mainArr[dropIndexes.column][dropIndexes.row], "add", this.component.getData('ltPropChildrenKey'), [elemData])
				objectUtils(popArr[dropIndexes.column][dropIndexes.row], "add", this.component.getData('ltPropChildrenKey'), [elemData])
				arrayUtils(mainArr, 'push', [[elemData]])
				arrayUtils(popArr, 'push', [[elemData]])
			}
		} else {
			if (dropIndexes.row > 0) {
				if (mainArr[dropIndexes.column][dropIndexes.row][this.component.getData('ltPropChildrenKey')]) {
					arrayUtils(this.component.getData('lyteOrgTreeMainArray')[dropIndexes.column][dropIndexes.row][this.component.getData('ltPropChildrenKey')], 'push', [elemData])
				} else {
					objectUtils(mainArr[dropIndexes.column][dropIndexes.row], "add", this.component.getData('ltPropChildrenKey'), [elemData])
				}
			}
			if ($L(event.target).closest('.lyteOrgTreeColumn')[0] == columns[elemIndex.column] && mainArr[dropIndexes.column][dropIndexes.row][this.component.getData('ltPropChildrenKey')]) {
				arrayUtils(mainArr[dropIndexes.column][dropIndexes.row][this.component.getData('ltPropChildrenKey')], "push", elemData);
			}
			else {
				var arrIndex = event.target.getAttribute('column-index');
				arrayUtils(mainArr[arrIndex - 1], 'push', elemData);
				arrayUtils(popArr[arrIndex - 1], 'push', elemData);
			}
		}
		this.component.setData('droppedOnSame', false)
		this.component.setData('dragRunning', false)
		arrayUtils(mainArr[elemIndex.column], 'removeAt', elemIndex.row, 1);
		arrayUtils(popArr[elemIndex.column], 'removeAt', elemIndex.row, 1);
...
LyteOrgTreeComponent._template = "<template tag-name=\"lyte-org-tree\"> <div> <div></div> <div></div> </div> <div class=\"lyteOrgTreeWrapper\"> <template items=\"{{lyteOrgTreePopulateArray}}\" item=\"item\" index=\"index\" is=\"for\" _new=\"true\"> <div class=\"lyteOrgTreeColumn {{if(expHandlers(index,'>=',expHandlers(lyteOrgTreePopulateArray.length,'-',ltPropMinimumExpanded)),'lyteOrgTreeExpandedColumn','lyteOrgTreeShrinkedColumn')}}\" column-index=\"{{expHandlers(index,'+',1)}}\" onscroll=\"{{action('scrollVerticalConnector',event,this,expHandlers(index,'+',1),item)}}\"> <div class=\"lyteOrgTreeScrollTopWrap\"> <div class=\"lyteOrgTreeTopScroll\" onclick=\"{{action('scrollColumnTop',index)}}\"></div> </div> <template items=\"{{item}}\" item=\"val\" index=\"ind\" is=\"for\" _new=\"true\"> <div class=\"lyteOrgTreeElementsWrap\" onclick=\"{{action('openChildLevel',this,val,index,event)}}\" onmousedown=\"{{action('startDrag',this)}}\" row-index=\"{{expHandlers(ind,'+',1)}}\"> <div class=\"lyteOrgTreeChildDropZone\"> <template is=\"switch\" l-c=\"true\" _new=\"true\"><template case=\"{{expHandlers(index,'>',0)}}\" is=\"case\" lc-id=\"lc_id_0\"><div class=\"lyteOrgTreeLeftHorizontalConnect\"></div></template></template><div class=\"lyteOrgTreeElements\" row-index=\"{{expHandlers(ind,'+',1)}}\"> <div class=\"lyteOrgTreeElementDragHandle\"></div> <template is=\"switch\" l-c=\"true\" _new=\"true\"><template case=\"{{expHandlers(index,'>=',expHandlers(lyteOrgTreePopulateArray.length,'-',ltPropMinimumExpanded))}}\" is=\"case\" lc-id=\"lc_id_0\"> <lyte-yield yield-name=\"orgTreeExpanded\" list-value=\"{{val}}\" class-value=\"lyteOrgTreeShrinked\"></lyte-yield> </template><template default=\"\"> <lyte-yield yield-name=\"orgTreeShrinked\" list-value=\"{{val}}\" class-value=\"lyteOrgTreeExpanded\"></lyte-yield> </template></template></div> <template is=\"switch\" l-c=\"true\" _new=\"true\"><template case=\"{{val[ltPropChildrenCountKey]}}\" is=\"case\" lc-id=\"lc_id_0\"><div class=\"lyteOrgTreeBadgeWrapper\"> <div class=\"lyteOrgTreeChildrenCountBadge\">{{val[ltPropChildrenCountKey]}}</div> </div></template></template><div class=\"lyteOrgTreeRightHorizontalConnect\"></div> <template is=\"switch\" l-c=\"true\" _new=\"true\"><template case=\"{{dragRunning}}\" is=\"case\" lc-id=\"lc_id_0\"><div class=\"lyteOrgTreeChildPlaceHolder\"></div></template></template></div> </div> </template> <div class=\"lyteOrgTreeLoadMoreWrap\"> <div class=\"lyteOrgTreeLoadMore lyteOrgTreeBottomScroll\" onclick=\"{{action('loadMoreData',index)}}\"></div> </div> <template is=\"switch\" l-c=\"true\" _new=\"true\"><template case=\"{{expHandlers(index,'>',0)}}\" is=\"case\" lc-id=\"lc_id_0\"><div class=\"lyteOrgTreeVerticalConnect\"></div></template></template><template is=\"switch\" l-c=\"true\" _new=\"true\"><template case=\"{{expHandlers(index,'>',0)}}\" is=\"case\" lc-id=\"lc_id_0\"><div class=\"lyteOrgTreeVerticalBorder\"></div></template></template></div> </template> </div> </template>";;
LyteOrgTreeComponent._dynamicNodes = [{"t":"a","p":[3,1]},{"t":"f","p":[3,1],"dN":[{"t":"a","p":[1]},{"t":"a","p":[1,1,1]},{"t":"a","p":[1,3]},{"t":"f","p":[1,3],"dN":[{"t":"a","p":[1]},{"t":"s","p":[1,1,1],"c":{"lc_id_0":{"dN":[],"cdp":{"t":"a","p":[0]},"dcn":true}},"d":{},"dc":{"lc_id_0":{}},"hd":true,"co":["lc_id_0"],"in":3,"sibl":[2]},{"t":"a","p":[1,1,2]},{"t":"s","p":[1,1,2,3],"c":{"lc_id_0":{"dN":[{"t":"a","p":[1],"cn":"lc_id_0"},{"t":"i","p":[1],"in":0,"cn":"lc_id_0"}],"cdp":{"t":"a","p":[0]},"dcn":true}},"d":{"dN":[{"t":"a","p":[1],"cn":"default"},{"t":"i","p":[1],"in":0,"cn":"default"}]},"dc":{"lc_id_0":{"dc":[0],"hc":true,"trans":true},"default":{"dc":[0],"hc":true,"trans":true}},"hd":true,"co":["lc_id_0"],"hc":true,"trans":true,"in":2,"sibl":[1]},{"t":"s","p":[1,1,4],"c":{"lc_id_0":{"dN":[{"t":"tX","p":[0,1,0],"cn":"lc_id_0"}],"cdp":{"t":"a","p":[0]},"dcn":true}},"d":{},"dc":{"lc_id_0":{}},"hd":true,"co":["lc_id_0"],"in":1,"sibl":[0]},{"t":"s","p":[1,1,7],"c":{"lc_id_0":{"dN":[],"cdp":{"t":"a","p":[0]},"dcn":true}},"d":{},"dc":{"lc_id_0":{}},"hd":true,"co":["lc_id_0"],"in":0}],"dc":[2],"hc":true,"trans":true,"in":2,"sibl":[1]},{"t":"a","p":[1,5,1]},{"t":"s","p":[1,7],"c":{"lc_id_0":{"dN":[],"cdp":{"t":"a","p":[0]},"dcn":true}},"d":{},"dc":{"lc_id_0":{}},"hd":true,"co":["lc_id_0"],"in":1,"sibl":[0]},{"t":"s","p":[1,8],"c":{"lc_id_0":{"dN":[],"cdp":{"t":"a","p":[0]},"dcn":true}},"d":{},"dc":{"lc_id_0":{}},"hd":true,"co":["lc_id_0"],"in":0}],"dc":[2],"hc":true,"trans":true,"in":0},{"type":"dc","trans":true,"hc":true,"p":[0]}];;

LyteOrgTreeComponent._observedAttributes = [
    "ltPropData",
    "ltPropChildrenKey",
    "ltPropTarget",
    "ltPropTargetIndex",
    "ltPropMinimumExpanded",
    "ltPropInitialRenderCount",
    "ltPropTargetKey",
    "ltPropChildrenCountKey",
    "ltPropDragThreshold",
    "lyteOrgTreeMainArray",
    "lyteOrgTreePopulateArray",
    "initialDimension",
    "dragRunning",
    "initialElemWrapDim",
    "currentDragInd",
...
export { LyteOrgTreeComponent };
LyteOrgTreeComponent.register("lyte-org-tree", {
    hash: "LyteOrgTreeComponent_2",
