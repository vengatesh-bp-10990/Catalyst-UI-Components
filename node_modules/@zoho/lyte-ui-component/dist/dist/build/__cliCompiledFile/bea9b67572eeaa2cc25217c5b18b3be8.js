( function( factory ) {
	...
	if($L){
        ...
        $L.prototype.draggable = function(object) {
            ...
            data.includePadding = data.includePadding ? data.includePadding : false;
                ...
                if(_handleElement){
                    ...
                    if(data && data._isDown){
                        ...
                        if(!data._isMoved){
                            ...
                            elem.style.top = scalePosition(elemOffset.top  - parentOffset.top /*- parseInt(cellSpacing)*/ - parseInt(_marginTop) + _scrollTop,data.scale.x) + 'px';
							elem.style.left = scalePosition(elemOffset.left - parentOffset.left - parseInt(_marginLeft) + _scrollLeft,data.scale.y) + 'px';
							if(data.scale.x){
                        ...
                        if(returnVal){
                            ...
                            if(orientation === "vertical"){
                                ...
                                elem.style.left = scalePosition(data._mousePosition.x  - _offset[0] - parentOffset.left - parseInt(_marginLeft) + _scrollLeft,data.scale.x) + 'px';
								elem.style.top = scalePosition(data._mousePosition.y  - _offset[1] - parentOffset.top - parseInt(_marginTop),data.scale.y)  + 'px';
                                elemOffset = elem.getBoundingClientRect();
                                if(data.scrollDivY){
                                    var scrollDivOffset = data.scrollDivY.getBoundingClientRect();
                                    if((elemOffset.left <= scrollDivOffset.right) && (elemOffset.right >= scrollDivOffset.left)){
                                ...
                                if(data.connectToSortable){
                                    ...
                                    AppendToDroppableParentCheck = function(){
                                        if($L(droppablePlace).hasClass('sortable-parent')){
                                            return droppablePlace._sortableParentData.droppable  && $L(data.connectToSortable).length &&  Array.from($L(data.connectToSortable)).indexOf(droppablePlace) !== -1;
                                        } 
                                    }
                                    ...
                                    if(droppablePlace && droppablePlace.parentElement._sortableParentData &&  droppablePlace.parentElement._sortableParentData.droppable /* && checkDroppable(droppablePlace,_sortableElem.parentElement,_sortableElem,data.connectedWith,data.containmentDimensions,_mousePosition)*/){

                                        if($L(_elemBelow).hasClass('sortable-parent') && checkParentDroppable(_elemBelow,data) && checkForIntersect(_elemBelow,data._mousePosition,data) && checkForBetween(_elemBelow,data._mousePosition,elem/*,isRelativeY,scrollDiv*/)){
                                    ...
                                    else if(_elemBelow && $L(_elemBelow).hasClass('sortable-parent') && checkParentDroppable(_elemBelow,data) && checkForIntersect(_elemBelow,data._mousePosition,data) && checkForBetween(_elemBelow,data._mousePosition,elem/*,isRelativeY,scrollDiv*/)){
                                        window._lyteUiUtils.appendChild(_elemBelow,_placeholder);
                                        _placeholder.style.display = "";
                                        _positionedPlceholder = true;
                                    }
                            ...
                            if(data.hasContainment && returnVal){
								...
								if(cs.position === 'relative' || cs.position === 'absolute' || cs.position === 'fixed'  || cs.position === 'static'){
            ...
            var checkForIntersect = function(parentElem,mP,data){
                var cs = window.getComputedStyle(parentElem);
                var offset = parentElem.getBoundingClientRect();
                if(!data.includePadding){
                    offset.left += parseFloat(cs.paddingLeft || 0);
                    offset.right -= parseFloat(cs.paddingRight || 0);
                    offset.top += parseFloat(cs.paddingTop || 0);
                    offset.bottom -= parseFloat(cs.paddingBottom || 0);
                }
                // console.log("cs",cs);
                // console.log("offset",offset);
                // console.log("_mousePosition",mP.x,mP.y);
                if(mP.x > offset.left && mP.x < offset.right  && mP.y > offset.top && mP.y < offset.bottom){
                    return true;
                }
                return false;
            };
            ...
            var checkForBetween = function(parentElem,mP,div){
                ...
                for(var i = 0;i<childrens.length;i++){
                    if(childrens[i].tagName != "TEMPLATE" && childrens[i].id != "lyteDraggableDummy"){
