		...
		function remove_duplicate( paths ){
			...
			for( var i = 0; i < len; i++ ){
				...
				if( is_hori == is_hori_next ){
					...
					len--;
					paths.splice( i-- + 2, 1 );
				} else if (is_hori && Math.abs(cur[x] - next[x]) <= 0.5) {//remove small difference
					var y = is_hori ? 'y' : 'x';
					cur[y] = next_after[y];
					len -= 2;
					paths.splice(i + 1, 2);
		...
		*/
		...
		$L.elbow = function( svg, start, end, data, ignore ){
			...
			if( existing_point && existing_point.length ){
				...
				if(options.minimizeBreakpoint){
					type = type == '5' ? '3' : type == '4' ? '2' : type;
				}
				...
				if( allow_break ){
					...
					var ranges = core_utils.deepCopyObject( data.getRanges(ignoreParent) ),
